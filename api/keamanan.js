  ...
  <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" bgcolor="#090c12" style="width:100%;margin:0;padding:0;background:#090c12;background-color:#090c12;background-image:linear-gradient(#090c12,#090c12)">
    <tr><td class="dirac-outer-pad" align="center" bgcolor="#090c12" style="padding:18px 12px;background:#090c12;background-color:#090c12;background-image:linear-gradient(#090c12,#090c12)">
      <table class="dirac-shell" role="presentation" width="600" cellspacing="0" cellpadding="0" border="0" bgcolor="#141820" style="width:100%;max-width:600px;border-collapse:separate;border-spacing:0;border:1px solid #2c3544;border-radius:18px;overflow:hidden;box-shadow:0 18px 48px rgba(0,0,0,.24);background:#141820;background-color:#141820;background-image:linear-gradient(#141820,#141820)">
        ...
        <tr><td style="padding:0 0 18px">${securityResetExecutiveReportHtmlV384('')}</td></tr>
        <tr><td style="padding:0;line-height:0;font-size:0"><table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="width:100%;border-collapse:collapse"><tr><td width="50%" height="4" bgcolor="#5276e8" style="height:4px;line-height:4px;font-size:0;background:#5276e8;background-color:#5276e8">&nbsp;</td><td width="30%" height="4" bgcolor="#148ba4" style="height:4px;line-height:4px;font-size:0;background:#148ba4;background-color:#148ba4">&nbsp;</td><td width="20%" height="4" bgcolor="#9a741f" style="height:4px;line-height:4px;font-size:0;background:#9a741f;background-color:#9a741f">&nbsp;</td></tr></table></td></tr>
      </table>
    </td></tr>
  </table>
...
function diracSecurityMailTextV327(input = {}) {
  ...
  return [
    ...
    '',
    securityResetExecutiveReportTextV384('')
...
async function securityResetHttpMailV352(provider, config, record, subject, text, html, reference) {
  ...
  try {
    ...
    const outbound = provider === 'brevo' ? {
      ...
      body: JSON.stringify({ sender: { name: 'PT Digdaya Inovasi Nusantara', email: config.brevoFrom }, to: [{ email: record.email }], subject, textContent: reportText, htmlContent: reportHtml, headers: { 'X-Dirac-Reference': reference } })
    } : {
      method: 'POST', redirect: 'error', signal: controller ? controller.signal : undefined,
      ...
      body: JSON.stringify({ from: 'PT Digdaya Inovasi Nusantara <' + config.resendFrom + '>', to: [record.email], subject, text: reportText, html: reportHtml })
    };
...
async function securityResetGmailMailV352(account, record, subject, text, html, slot) {
  ...
  const mime = [
    'From: PT Digdaya Inovasi Nusantara Security <' + account.user + '>', 'To: ' + record.email,
...
async function securityResetSendCommittedPasswordMailV342(record, req) {
  ...
  const subject = 'PT Digdaya Inovasi Nusantara Security - Password Berhasil Diganti [' + reference + ']';
  ...
  const htmlInput = {
    preheader: 'Password akun PT Digdaya Inovasi Nusantara berhasil diganti.', brandLabel: 'SECURE ACCOUNT SECURITY', eyebrow: 'PASSWORD SECURITY NOTICE',
    title: 'Password Berhasil\nDiganti', greeting: 'Yth. Pengguna PT Digdaya Inovasi Nusantara,',
    summary,
    statusLabel: 'STATUS PASSWORD', statusValue: 'PASSWORD BARU AKTIF', statusNote: 'Perubahan ini telah dikonfirmasi oleh server keamanan PT Digdaya Inovasi Nusantara.',
    ...
    warningTitle: 'PERIKSA JIKA BUKAN ANDA', warning: 'Jika Anda tidak melakukan perubahan ini, segera tinjau keamanan akun dan hubungi kanal resmi PT Digdaya Inovasi Nusantara. Jangan bagikan password, OTP, token, cookie, kode keamanan, atau Passkey.',
    supportLead: 'Jika membutuhkan bantuan terkait keamanan password, gunakan kanal resmi PT Digdaya Inovasi Nusantara berikut.'
  };
...
async function diracSecurityPasskeyResetSendEmailCodeV364(req, owner, code, payload) {
  ...
  const subject = 'PT Digdaya Inovasi Nusantara Security - Kode Verifikasi Ganti Passkey [' + reference + ']';
  const client = diracSecurityMailRequestContextV381(req);
  const htmlInput = {
    preheader: 'Kode verifikasi satu kali untuk mengganti Passkey akun PT Digdaya Inovasi Nusantara.', brandLabel: 'SECURE PASSKEY REPLACEMENT', eyebrow: 'PASSKEY VERIFICATION CODE',
    title: 'Kode Verifikasi\nGanti Passkey', greeting: 'Yth. Pengguna PT Digdaya Inovasi Nusantara,',
...
async function diracSecurityPasskeyResetSendChangedNoticeV372(req, owner, rotationId, securityEpoch) {
  ...
  const subject = 'PT Digdaya Inovasi Nusantara Security - Passkey Berhasil Diganti [' + reference + ']';
  const client = diracSecurityMailRequestContextV381(req);
  const htmlInput = {
...
async function diracPasswordChangeSendSmtpCodeV365(req, owner, code, payload) {
  ...
  const reference = diracSecurityPasskeyResetSha256V363(String(payload.jti || '')).slice(0, 10).toUpperCase();
  const subject = 'PT Digdaya Inovasi Nusantara Security - Kode Ganti Kata Sandi [' + reference + ']';
  const client = diracSecurityMailRequestContextV381(req);
  const htmlInput = {
    preheader: 'Kode SMTP satu kali untuk mengganti kata sandi akun PT Digdaya Inovasi Nusantara.', brandLabel: 'SECURE PASSWORD CHANGE', eyebrow: 'PASSWORD VERIFICATION CODE',
    title: 'Kode Verifikasi\nGanti Kata Sandi', greeting: 'Yth. Pengguna PT Digdaya Inovasi Nusantara,',
