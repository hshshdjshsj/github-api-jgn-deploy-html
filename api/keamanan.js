...
const SECURITY_RESET_REQUEST_CONTEXT_V361 = new AsyncLocalStorage();
const SECURITY_RESET_EGRESS_CAPABILITIES_V361 = new WeakMap();
...
async function openD10Request(carrier, ops) {
  ...
  try {
    ...
    let plainText;
    let inner;
    try { plainText = decoder.decode(current); inner = parseStrictJson(plainText, { maxBytes: SECURITY_RESET_MAX_BODY_V334 }); }
  ...
  try {
    ...
    if (inner.op === 'start') {
      ...
      if (authPasskeyFlow) {
        ...
        if (!Array.isArray(allowCredentials) || allowCredentials.length !== 1) throw resetError('PASSWORD_RESET_PASSKEY_AMBIGUOUS', 403);
        const deviceBindingContext = await ops.deviceBindingContext(email, String(allowCredentials[0] && allowCredentials[0].id || ''));
        diracResetDiagnosticV335(req, 'start.challenge_issue', 'begin', { rp_id: rpId, email_binding_hash: diracPasswordResetBindingHashV333('diag-email', email).slice(0, 20), allow_credentials_count: allowCredentials.length, device_binding_policy: String(deviceBindingContext.policy || '') });
        const issued = await ops.issueChallenge({ email, browserHash, requestBinding: bindingNow, keyId, rpId, serverDeviceBindingKeyId: String(deviceBindingContext.keyId || ''), deviceBindingPolicy: String(deviceBindingContext.policy || '') });
        ...
        payload = {
          ...
          publicKey: {
            ...
            device_binding_policy: String(deviceBindingContext.policy || '')
          }
        };
      ...
      if (authPasskeyFlow) {
        ...
        const verifyCredentialId = diracPasskeyA2FCredentialId(inner.credential);
        const deviceBindingContext = await ops.deviceBindingContext('', verifyCredentialId);
        diracResetDiagnosticV335(req, 'verify.challenge_read', 'begin', { challenge_id_hash: crypto.createHash('sha256').update(challengeId).digest('hex').slice(0, 20), device_binding_policy: String(deviceBindingContext.policy || '') });
        const state = await ops.readChallenge(challengeId, { browserHash, requestBinding: bindingNow, keyId, rpId: String(await ops.rpId()), serverDeviceBindingKeyId: String(deviceBindingContext.keyId || ''), deviceBindingPolicy: String(deviceBindingContext.policy || '') });
        ...
        await ops.emailRate('success', resolved.owner.customerId, 1, 1, 1);
...
const DIRAC_PASSKEY_SERVER_DEVICE_BINDING_VERSION_V367 = 'dirac-passkey-server-device-binding-v2';
const DIRAC_PASSKEY_SERVER_DEVICE_BINDING_ALGORITHM_V367 = 'HMAC-SHA256-HTTPONLY-DEVICE';
const DIRAC_PASSKEY_SERVER_DEVICE_BINDING_POLICY_V367 = 'httponly-server-bound-v2';
...
function requestOrigin(req) {
  ...
  const origin = normalizeDashboardMfaOrigin(raw);
...
function diracCentralCurrentContextV149() { return SECURITY_RESET_REQUEST_CONTEXT_V361.getStore() || null; }
...
function diracResetDiagnosticErrorV335(error) {
  ...
  return {
    ...
    // Stack messages can embed provider response details or sensitive input.
    category: 'security_reset_error'
  };
}
...
async function securityResetPasskeyV237DbContractDiagnosticV370(req, resolved, payload, activeRow, createdResult) {
  try {
    ...
    diracResetDiagnosticV335(req, 'passkey_reset.v237_db_contract', 'diagnostic', {
      ...
      diagnosis,
      rpc_anchor_source: 'token.rpcAuthSessionId',
...
function securityResetValidateBrowserV334(req, method) {
  ...
  if (context && context.req === req) context.browserChecked = true;
  return origin;
}
...
function securityResetValidateHeadersV361(req) {
  ...
  let bytes = 0;
  for (const [name, value] of Object.entries(req && req.headers || {})) {
    if ((critical.has(name.toLowerCase()) && Array.isArray(value)) || /[\r\n\0]/.test(String(value || ''))) throw resetError('SECURITY_RESET_HEADER_INVALID', 400);
    if (['host', 'x-forwarded-host', 'x-forwarded-proto'].includes(name.toLowerCase()) && String(value).includes(',')) throw resetError('SECURITY_RESET_HEADER_INVALID', 400);
    bytes += Buffer.byteLength(name + ':' + String(value || ''));
  }
  if (bytes > 65536) throw resetError('SECURITY_RESET_HEADERS_TOO_LARGE', 431);
}
async function securityResetReadJsonV334(req) {
  const contentType = securityResetHeaderV334(req, 'content-type').trim().toLowerCase();
  if (!/^application\/json(?:\s*;\s*charset=utf-8)?$/.test(contentType)) throw resetError('SECURITY_RESET_CONTENT_TYPE_INVALID', 415);
  if (securityResetHeaderV334(req, 'transfer-encoding').trim()) throw resetError('SECURITY_RESET_TRANSFER_ENCODING_REJECTED', 400);
  const encoding = securityResetHeaderV334(req, 'content-encoding').trim().toLowerCase();
  if (encoding && encoding !== 'identity') throw resetError('SECURITY_RESET_CONTENT_ENCODING_REJECTED', 415);
...
function securitySupabaseCredentialsV334(target) {
  ...
  if (u.protocol !== 'https:' || !/^[a-z0-9-]+\.supabase\.co$/.test(u.hostname) || u.pathname !== '/') throw resetError('SECURITY_RESET_DB_URL_INVALID', 503);
...
function securityResetAssertDbOperationV361(path, method, options) {
  const context = diracCentralCurrentContextV149();
  if (!context || !context.active || !context.browserChecked || !context.banChecked
      || Date.now() - context.startedAt > 120000) throw resetError('SECURITY_RESET_DB_CONTEXT_REQUIRED', 503);
  const url = new URL('https://reset.invalid' + path);
  if (url.pathname + url.search !== path || url.hash || /%(?:2f|5c|2e|00)/i.test(url.pathname)) throw resetError('SECURITY_RESET_DB_PATH_INVALID', 403);
  const queryKeys = new Set();
  for (const [key, value] of url.searchParams) {
    if (queryKeys.has(key) || !/^[a-z][a-z0-9_]{0,62}$/.test(key) || /[\r\n\0]/.test(value)) throw resetError('SECURITY_RESET_DB_QUERY_INVALID', 403);
    queryKeys.add(key);
  }
  const mutationFields = {
    '/rest/v1/domain_passkeys': ['is_active', 'revoked_at', 'revoke_reason', 'updated_at'],
    '/rest/v1/security_customer_sessions': ['status', 'revoked_at', 'revoke_reason'],
    '/rest/v1/security_customer_password_hashes': ['auth_user_id', 'customer_id', 'email_hash', 'password_hash', 'hash_algorithm', 'hash_params', 'status', 'created_at', 'updated_at'],
    '/rest/v1/dirac_persistent_bans': ['security_key', 'record_json', 'blocked_until_ms', 'updated_at', 'expires_at']
  };
  if (url.pathname.startsWith('/rest/v1/rpc/')) {
    ...
    if (name === 'dirac_passkey_create_pending_v237' || name === 'dirac_passkey_finalize_rotation_v237') {
      ...
      if (!owner || context.__diracPasskeyResetAuthorizedV363 !== true
          || context.__diracPasskeyResetAuthorizationMethodV364 !== 'password_email'
          || !/^[a-f0-9]{64}$/.test(String(context.__diracPasskeyResetEmailVerificationHashV364 || ''))
          ...
          || !/^[a-f0-9]{64}$/.test(String(context.__diracPasskeyResetAuthorizingCredentialHashV363 || ''))
  ...
  if (url.pathname.startsWith('/auth/v1/admin/users/')) {
    ...
    const owner = context.__diracPasswordResetVerifiedOwnerV333;
    const expected = owner && owner.authUserId || context.__diracPasswordResetExpectedAuthUserIdV333;
    if (!customerSecurityLooksLikeUuid(userId) || !expected || userId !== expected || !['GET', 'PUT'].includes(method)) throw resetError('SECURITY_RESET_DB_OWNER_REQUIRED', 403);
    if (method === 'PUT' && (!context.__diracPasswordResetGrantConsumedV361 || !exactKeys(options.body, ['password']))) throw resetError('SECURITY_RESET_DB_GRANT_REQUIRED', 403);
    return;
  }
  ...
  const passkeySessionContinuityV372 = context.__diracPasskeyResetSessionContinuityV372;
  if (url.pathname === '/rest/v1/security_customer_sessions' && method === 'PATCH' && passkeySessionContinuityV372 && passkeySessionContinuityV372.active === true) {
    const owner = context.__diracPasswordResetVerifiedOwnerV333;
    const sessionId = String(passkeySessionContinuityV372.sessionId || '');
    const securityEpoch = Number(passkeySessionContinuityV372.securityEpoch || 0);
    if (!owner || !customerSecurityLooksLikeUuid(String(owner.customerId || '')) || !customerSecurityLooksLikeUuid(sessionId)
        ...
        || url.searchParams.get('status') !== 'eq.revoked'
        || url.searchParams.get('security_epoch') !== 'eq.' + String(securityEpoch)
        || url.searchParams.get('revoke_reason') !== 'eq.passkey_rotation_v237'
        || options.prefer !== 'return=representation'
        || !exactKeys(options.body, ['status', 'revoked_at', 'revoke_reason'])
        || options.body.status !== 'active' || options.body.revoked_at !== null || options.body.revoke_reason !== null) {
      throw resetError('SECURITY_PASSKEY_RESET_SESSION_CONTINUITY_DB_REJECTED', 403);
    }
    return;
  }
  const passkeySessionTokenRebindV381 = context.__diracPasskeyResetSessionTokenRebindV381;
  if (url.pathname === '/rest/v1/security_customer_sessions' && method === 'PATCH' && passkeySessionTokenRebindV381 && passkeySessionTokenRebindV381.active === true) {
    const owner = context.__diracPasswordResetVerifiedOwnerV333;
    const sessionId = String(passkeySessionTokenRebindV381.sessionId || '');
    const securityEpoch = Number(passkeySessionTokenRebindV381.securityEpoch || 0);
    const sessionTokenHash = String(passkeySessionTokenRebindV381.sessionTokenHash || '');
    const lastSeenMs = Date.parse(String(options.body && options.body.last_seen_at || ''));
    if (!owner || !customerSecurityLooksLikeUuid(String(owner.customerId || '')) || !customerSecurityLooksLikeUuid(sessionId)
        || !Number.isSafeInteger(securityEpoch) || securityEpoch < 2 || !/^[a-f0-9]{64}$/.test(sessionTokenHash)
        || url.searchParams.size !== 5
        || url.searchParams.get('customer_id') !== 'eq.' + owner.customerId
        || url.searchParams.get('id') !== 'eq.' + sessionId
        ...
        || options.prefer !== 'return=representation'
        || !exactKeys(options.body, ['session_token_hash', 'last_seen_at'])
        || !safeEqual(String(options.body.session_token_hash || ''), sessionTokenHash)
        || !Number.isFinite(lastSeenMs) || Math.abs(Date.now() - lastSeenMs) > 30000) {
      throw resetError('SECURITY_PASSKEY_RESET_SESSION_REBIND_DB_REJECTED', 403);
    }
    return;
  }
  ...
  if (!readTables.includes(url.pathname.slice('/rest/v1/'.length))) throw resetError('SECURITY_RESET_DB_TABLE_REJECTED', 403);
  if (method === 'GET') return;
  const allowed = mutationFields[url.pathname];
  if (!allowed || !['POST', 'PATCH', 'DELETE'].includes(method)) throw resetError('SECURITY_RESET_DB_METHOD_INVALID', 403);
  if (options.body !== undefined) {
    const rows = Array.isArray(options.body) ? options.body : [options.body];
    if (rows.length !== 1 || rows.some(row => !row || typeof row !== 'object' || Array.isArray(row) || Object.keys(row).some(key => !allowed.includes(key)))) throw resetError('SECURITY_RESET_DB_FIELDS_REJECTED', 403);
  }
  if (url.pathname === '/rest/v1/dirac_persistent_bans') {
    const rows = Array.isArray(options.body) ? options.body : [];
    const smtpRate = context.__diracSecuritySmtpRequestRateV374;
    if (smtpRate && smtpRate.active === true) {
      const row = rows.length === 1 ? rows[0] : null, record = row && row.record_json;
      const updatedAtMs = Date.parse(String(row && row.updated_at || ''));
      if (method !== 'POST' || url.searchParams.size !== 1 || url.searchParams.get('on_conflict') !== 'security_key' || !row || !record
          || !exactKeys(row, ['security_key','record_json','blocked_until_ms','updated_at','expires_at'])
          || !exactKeys(record, ['type','flow','auth_user_id','customer_id','attempt_count','blocked_until_ms','updated_at_ms'])
          || String(row.security_key || '') !== String(smtpRate.key || '') || !/^security-smtp-request-rate-v374:[a-f0-9]{64}$/.test(String(row.security_key || ''))
          || String(record.type || '') !== DIRAC_SECURITY_SMTP_REQUEST_RATE_V374 || String(record.flow || '') !== String(smtpRate.flow || '')
          || String(record.auth_user_id || '') !== String(smtpRate.authUserId || '') || String(record.customer_id || '') !== String(smtpRate.customerId || '')
          || !Number.isSafeInteger(Number(record.attempt_count)) || Number(record.attempt_count) < 0 || Number(record.attempt_count) > 5
          || Number(record.attempt_count) !== Number(smtpRate.attemptCount) || !Number.isSafeInteger(Number(record.blocked_until_ms)) || Number(record.blocked_until_ms) < 0
          || Number(record.blocked_until_ms) !== Number(smtpRate.blockedUntilMs) || !Number.isSafeInteger(Number(record.updated_at_ms)) || Number(record.updated_at_ms) <= 0
          || !Number.isFinite(updatedAtMs) || Math.abs(updatedAtMs - Number(record.updated_at_ms)) > 1000
          || Number(row.blocked_until_ms) !== Number(smtpRate.blockedUntilMs) || String(row.expires_at || '') !== '9999-01-01T00:00:00.000Z') throw resetError('SECURITY_SMTP_REQUEST_RATE_DB_REJECTED', 403);
      return;
    }
    if (method !== 'POST' || url.searchParams.get('on_conflict') !== 'security_key' || rows.length !== 1
        || !/^password-reset-mail(?:-dispatch)?-v338:[a-f0-9]{64}$/.test(String(rows[0].security_key || ''))
        || !context.__diracPasswordResetGrantConsumedV361) throw resetError('SECURITY_RESET_DB_MAIL_SCOPE_INVALID', 403);
    return;
  }
  const owner = context.__diracPasswordResetVerifiedOwnerV333;
  if (!owner) throw resetError('SECURITY_RESET_DB_OWNER_REQUIRED', 403);
  const ownerKey = url.pathname === '/rest/v1/domain_passkeys' ? 'user_id'
    : url.pathname === '/rest/v1/security_customer_password_hashes' ? 'auth_user_id' : 'customer_id';
  const expectedId = ownerKey === 'auth_user_id' ? owner.authUserId : owner.customerId;
  if (method !== 'POST' && url.searchParams.get(ownerKey) !== 'eq.' + expectedId) throw resetError('SECURITY_RESET_DB_OWNER_REQUIRED', 403);
  if (method === 'POST') {
    const rows = Array.isArray(options.body) ? options.body : [options.body];
    if (rows.some(row => row[ownerKey] !== expectedId)) throw resetError('SECURITY_RESET_DB_OWNER_REQUIRED', 403);
  }
}

function securityResetEgressCapabilityV361(url, options, purpose) {
  const context = diracCentralCurrentContextV149();
  if (!context || !context.active || !context.browserChecked || !context.banChecked) throw resetError('SECURITY_RESET_EGRESS_CONTEXT_REQUIRED', 503);
  ...
  SECURITY_RESET_EGRESS_CAPABILITIES_V361.set(capability, Object.freeze({
    request: context.req, url, method: options.method, headers: JSON.stringify(options.headers),
    body: crypto.createHash('sha256').update(String(options.body || '')).digest('hex'), expires: Date.now() + 10000
  }));
  return capability;
}

async function securityResetEgressFetchV361(url, options, capability) {
  const permit = SECURITY_RESET_EGRESS_CAPABILITIES_V361.get(capability);
  SECURITY_RESET_EGRESS_CAPABILITIES_V361.delete(capability);
  const context = diracCentralCurrentContextV149();
  if (!permit || !context || !context.active || permit.request !== context.req || permit.expires <= Date.now()
      || permit.url !== url || permit.method !== options.method || permit.headers !== JSON.stringify(options.headers)
      || permit.body !== crypto.createHash('sha256').update(String(options.body || '')).digest('hex')) throw resetError('SECURITY_RESET_EGRESS_CAPABILITY_REJECTED', 503);
  if (!SECURITY_NATIVE_FETCH_V334) throw resetError('SECURITY_RESET_NATIVE_FETCH_UNAVAILABLE', 503);
  return SECURITY_NATIVE_FETCH_V334(url, { ...options, redirect: 'error' });
}
...
async function securityResetReadUpstreamV361(response, maxBytes = 2097152) {
  ...
  try { return parseStrictJson(raw, { maxBytes, maxNodes: 20000, maxDepth: 20 }); }
  catch (_) { throw resetError('SECURITY_RESET_UPSTREAM_JSON_INVALID', 503); }
}
...
async function supabaseFetch(path, options = {}) {
  ...
  securityResetAssertDbOperationV361(cleanPath, method, options);
  ...
  let response, data;
  try {
    const outbound = { method, headers, body, redirect: 'error', signal: controller.signal };
    const capability = securityResetEgressCapabilityV361(c.url + cleanPath, outbound, 'database');
    response = await securityResetEgressFetchV361(c.url + cleanPath, outbound, capability);
    data = await securityResetReadUpstreamV361(response);
  }
...
async function securityResetRateLimitV334(req, action) {
  ...
  const key = 's2s-central-rate-v230:' + diracCentralHashV146(['keamanan-reset-v361',action,ip].join('|'));
...
function diracPasskeyA2FDecodeClientData(value){const b=diracPasskeyA2FBase64UrlToBuffer(value);if(!b.length||b.length>8192)return null;try{return parseStrictJson(b,{maxBytes:8192,maxDepth:4,maxNodes:64});}catch(_){return null;}}
...
function diracPasskeyA2FStoredDeviceBinding(row){const j=row&&row.credential_json&&typeof row.credential_json==='object'?row.credential_json:{},b=j.device_binding&&typeof j.device_binding==='object'?j.device_binding:null;if(!b||b.required!==true||!/^[a-f0-9]{64}$/.test(String(b.key_id||'')))return{ok:false,reason:'stored_device_binding_missing'};if(b.version===DIRAC_PASSKEY_SERVER_DEVICE_BINDING_VERSION_V367&&b.algorithm===DIRAC_PASSKEY_SERVER_DEVICE_BINDING_ALGORITHM_V367&&b.policy===DIRAC_PASSKEY_SERVER_DEVICE_BINDING_POLICY_V367)return{ok:true,keyId:String(b.key_id),binding:b,version:DIRAC_PASSKEY_SERVER_DEVICE_BINDING_VERSION_V367,algorithm:DIRAC_PASSKEY_SERVER_DEVICE_BINDING_ALGORITHM_V367,policy:DIRAC_PASSKEY_SERVER_DEVICE_BINDING_POLICY_V367,serverBound:true};if(b.version!==DIRAC_PASSKEY_DEVICE_BINDING_VERSION||b.algorithm!==DIRAC_PASSKEY_DEVICE_BINDING_ALGORITHM)return{ok:false,reason:'stored_device_binding_policy_invalid'};const k=diracPasskeyA2FDeviceBindingKeyId(b.public_key_jwk);return k&&safeEqual(k,String(b.key_id))?{ok:true,keyId:k,publicKeyJwk:b.public_key_jwk,binding:b,version:DIRAC_PASSKEY_DEVICE_BINDING_VERSION,algorithm:DIRAC_PASSKEY_DEVICE_BINDING_ALGORITHM,policy:'webcrypto-nonextractable-v1',serverBound:false}:{ok:false,reason:'stored_device_binding_key_invalid'};}
...
function diracPasskeyA2FValidateAuthenticationDeviceBinding({row,body,setupToken,payload,credentialId,req,owner}){const stored=diracPasskeyA2FStoredDeviceBinding(row);if(!stored.ok)return stored;if(stored.serverBound===true){if(diracPasskeyA2FSafeString(body&&body.response,64)!=='dpk2s')return{ok:false,reason:'server_device_binding_marker_invalid'};const expected=String(payload&&payload.serverDeviceBindingKeyId||'');if(!/^[a-f0-9]{64}$/.test(expected))return{ok:false,reason:'server_device_binding_challenge_invalid'};const current=diracSecurityPasskeyResetServerDeviceAuthorityV367().read(req,owner);if(!current||current.ok!==true)return{ok:false,reason:String(current&&current.reason||'server_device_binding_cookie_invalid')};if(!safeEqual(expected,String(current.keyId||''))||!safeEqual(stored.keyId,String(current.keyId||'')))return{ok:false,reason:'server_device_binding_key_mismatch'};return{ok:true,keyId:stored.keyId,binding:stored.binding,version:stored.version,algorithm:stored.algorithm,policy:stored.policy,serverBound:true};}const parts=diracPasskeyA2FSafeString(body&&body.response,1024).split('.');if(parts.length!==3||parts[0]!=='dpk1a'||!/^[a-f0-9]{64}$/.test(parts[1])||!/^[A-Za-z0-9_-]{86}$/.test(parts[2]))return{ok:false,reason:'device_binding_authentication_envelope_invalid'};if(!safeEqual(stored.keyId,parts[1]))return{ok:false,reason:'device_binding_key_mismatch'};const sig=diracPasskeyA2FBase64UrlToBuffer(parts[2]);if(sig.length!==64)return{ok:false,reason:'device_binding_signature_format_invalid'};try{const key=crypto.createPublicKey({key:stored.publicKeyJwk,format:'jwk'});const input=diracPasskeyA2FDeviceBindingSigningInput({setupToken,payload,credentialId,mode:'authentication'});const ok=crypto.verify('sha256',Buffer.from(input),{key,dsaEncoding:'ieee-p1363'},sig);return ok?{ok:true,keyId:stored.keyId,publicKeyJwk:stored.publicKeyJwk,binding:stored.binding,version:stored.version,algorithm:stored.algorithm,policy:stored.policy,serverBound:false}:{ok:false,reason:'device_binding_authentication_signature_invalid'};}catch(_){return{ok:false,reason:'device_binding_authentication_signature_invalid'};}}
...
function diracPasswordResetSealX25519PrivateV333(rawPrivate) {
  ...
  const fullMinute = Math.floor(Date.now() / 60000);
  const minute = fullMinute & 0xffff;
  const minuteBytes = Buffer.alloc(2);
  minuteBytes.writeUInt16BE(minute, 0);
  const nonce = crypto.randomBytes(10);
  const key = Buffer.from(diracCentralDeriveSecretV146('password-reset-d10-x25519-v361:' + fullMinute)).subarray(0, 32);
  const aad = Buffer.from(DIRAC_PASSWORD_RESET_V333.protocol + '\n' + DIRAC_PASSWORD_RESET_V333.suite + '\n' + String(fullMinute), 'utf8');
  try {
    const cipher = crypto.createCipheriv('aes-256-gcm', key, Buffer.concat([minuteBytes, nonce]), { authTagLength: 16 });
    cipher.setAAD(aad);
...
function diracPasswordResetUnsealX25519PrivateV333(keyId) {
  ...
  const currentFullMinute = Math.floor(Date.now() / 60000);
  const current = currentFullMinute & 0xffff;
  ...
  const fullMinute = currentFullMinute - forward;
...
function diracPasswordResetChallengeTagV333(prefix, email, context) {
  ...
  const material = Buffer.from([
    ...
    String(source.keyId || ''),
    String(source.rpId || ''),
    String(source.serverDeviceBindingKeyId || ''),
    String(source.deviceBindingPolicy || '')
  ].join('\n'), 'utf8');
  try {
...
function diracPasswordResetReadChallengeV333(challengeId, context) {
  ...
  const consumeBinding = diracPasswordResetBindingHashV333('consume', [
    ...
    String(source.serverDeviceBindingKeyId || ''), String(source.deviceBindingPolicy || '')
  ].join('\n'));
  return Object.freeze({
    ...
    server_device_binding_key_id: String(source.serverDeviceBindingKeyId || ''),
    device_binding_policy: String(source.deviceBindingPolicy || '')
  });
}
...
function diracPasswordResetChallengeOwnerValidV333(state, email) {
  ...
  try {
    ...
    expected = diracPasswordResetChallengeTagV333(prefix, normalizeAuthEmail(email || ''), {
      ...
      keyId: state.key_id,
      rpId: state.rp_id,
      serverDeviceBindingKeyId: state.server_device_binding_key_id,
      deviceBindingPolicy: state.device_binding_policy
    });
...
async function diracPasswordResetDeviceBindingContextV374(req, emailValue, credentialId) {
  const row = await diracPasswordResetFetchCredentialByIdV333(credentialId, '');
  const rowEmail = normalizeAuthEmail(row && row.email || '');
  const expectedEmail = normalizeAuthEmail(emailValue || rowEmail);
  if (!isValidAuthEmail(expectedEmail) || !safeEqual(rowEmail, expectedEmail)) throw diracPasswordResetErrorV333('PASSWORD_RESET_EMAIL_PASSKEY_MISMATCH', 403);
  const owner = await diracPasswordResetResolveOwnerV333(req, row, diracPasswordResetBindingHashV333('email', rowEmail));
  if (!safeEqual(String(row.user_id || ''), owner.customerId) || !safeEqual(rowEmail, owner.email)) throw diracPasswordResetErrorV333('PASSWORD_RESET_PASSKEY_OWNER_INVALID', 403);
  const stored = diracPasskeyA2FStoredDeviceBinding(row);
  if (!stored.ok) throw diracPasswordResetErrorV333(String(stored.reason || 'STORED_DEVICE_BINDING_INVALID').toUpperCase(), 403);
  if (stored.serverBound === true) {
    const current = diracSecurityPasskeyResetServerDeviceAuthorityV367().read(req, owner);
    if (!current || current.ok !== true || !safeEqual(String(current.keyId || ''), stored.keyId)) throw diracPasswordResetErrorV333('SERVER_DEVICE_BINDING_KEY_MISMATCH', 403);
    return Object.freeze({ policy: DIRAC_PASSKEY_SERVER_DEVICE_BINDING_POLICY_V367, keyId: stored.keyId });
  }
  return Object.freeze({ policy: 'webcrypto-nonextractable-v1', keyId: '' });
}
...
async function diracPasswordResetVerifyPasskeyV333(req, state, input) {
  ...
  const payload = { challenge: String(state.challenge || ''), rpId, serverDeviceBindingKeyId: String(state.server_device_binding_key_id || ''), deviceBindingPolicy: String(state.device_binding_policy || '') };
  const storedDeviceBinding = diracPasskeyA2FStoredDeviceBinding(row);
  if (!storedDeviceBinding.ok || !safeEqual(String(state.device_binding_policy || ''), String(storedDeviceBinding.policy || ''))
      || (storedDeviceBinding.serverBound === true && !safeEqual(String(state.server_device_binding_key_id || ''), storedDeviceBinding.keyId))
      || (storedDeviceBinding.serverBound !== true && String(state.server_device_binding_key_id || '') !== '')) throw diracPasswordResetErrorV333('PASSWORD_RESET_DEVICE_BINDING_STATE_CHANGED', 403);
...
const SECURITY_RESET_MAIL_DIAGNOSTICS_V340 = new WeakMap();
function securityResetMailDiagnosticV340(req, stage, outcome, details) {
  try {
    const diagnostic = SECURITY_RESET_MAIL_DIAGNOSTICS_V340.get(req);
    if (!diagnostic || !['persist.begin', 'persist.result', 'persist.exception', 'dispatch.begin', 'dispatch.preflight', 'dispatch.result', 'dispatch.exception', 'dispatch.lifecycle'].includes(stage)
        || !['begin', 'success', 'error', 'scheduled', 'awaited'].includes(outcome)) return;
    const input = details && typeof details === 'object' ? details : {};
    const safe = {};
    for (const name of ['ok', 'delivered', 'registry_ok', 'revocations_ok', 'server_id_valid', 'key_version_valid', 'root_present', 'root_valid', 'record_confirmed', 'notice_present']) if (typeof input[name] === 'boolean') safe[name] = input[name];
    for (const name of ['status', 'upstream_status', 'registry_status', 'revocations_status', 'registry_rows', 'revocations_rows', 'record_rows', 'stage_elapsed_ms']) if (Number.isFinite(input[name])) safe[name] = Math.max(-1, Math.min(86400000, Math.trunc(input[name])));
    if (['request_wait_until', 'global_wait_until', 'handler_await'].includes(input.mode)) safe.mode = input.mode;
    if (['brevo', 'resend', 'gmail_smtp'].includes(input.provider)) safe.provider = input.provider;
    if (input.code) safe.code = ['PASSWORD_RESET_MAIL_ROOT_INVALID', 'PASSWORD_RESET_MAIL_RECORD_UNCONFIRMED', 'PASSWORD_RESET_MAIL_CONFIGURATION_UNAVAILABLE', 'PASSWORD_RESET_MAIL_PREFLIGHT_UNAVAILABLE', 'PASSWORD_RESET_MAIL_SELF_SCOPE_UNAVAILABLE', 'PASSWORD_RESET_MAIL_INPUT_INVALID', 'PASSWORD_RESET_MAIL_FULL_GUARD_REQUIRED', 'PASSWORD_RESET_MAIL_CLAIM_UNAVAILABLE_OR_USED', 'PASSWORD_RESET_MAIL_UNCONFIRMED', 'PASSWORD_RESET_MAIL_CENTRAL_GUARD_REJECTED'].includes(input.code) ? input.code : 'UNCLASSIFIED_ERROR';
    console.info('[dirac-password-mail-diagnostic-v340] ' + JSON.stringify({ v: 'dirac-password-mail-diagnostic-v340', ts: new Date().toISOString(),
      trace_id: diagnostic.traceId, correlation_id: diagnostic.correlationId, component: 'keamanan', stage, outcome,
...
async function securityResetRememberCommittedMailV338(req, owner, state, grantId) {
  ...
  securityResetMailDiagnosticV340(req, 'persist.begin', 'begin', { root_present: Boolean(notificationRoot), root_valid: Buffer.byteLength(notificationRoot, 'utf8') >= minimumRootBytes });
  ...
  securityResetMailDiagnosticV340(req, 'persist.result', write && write.ok === true ? 'success' : 'error', { ok: Boolean(write && write.ok === true), status: Number(write && write.status || 0), record_rows: Array.isArray(write && write.data) ? write.data.length : -1, stage_elapsed_ms: Date.now() - now });
...
function diracSecurityMailSafeRequestUrlV381(value) {
  try {
    const parsed = new URL(String(value || '').trim());
    if (!['https:', 'http:'].includes(parsed.protocol)) return 'Tidak tersedia';
    return diracSecurityMailCleanV327(parsed.origin + parsed.pathname, 500) || 'Tidak tersedia';
  } catch (_) { return 'Tidak tersedia'; }
}
function diracSecurityMailBrowserDetailV381(userAgent) {
  const ua = String(userAgent || '').slice(0, 1000);
  const edge = ua.match(/Edg\/([0-9.]+)/i), opera = ua.match(/OPR\/([0-9.]+)/i), crios = ua.match(/CriOS\/([0-9.]+)/i);
  const chrome = ua.match(/Chrome\/([0-9.]+)/i), fxios = ua.match(/FxiOS\/([0-9.]+)/i), firefox = ua.match(/Firefox\/([0-9.]+)/i), safari = ua.match(/Version\/([0-9.]+).*Safari\//i);
  if (edge) return 'Microsoft Edge ' + edge[1]; if (opera) return 'Opera ' + opera[1]; if (crios) return 'Chrome iOS ' + crios[1];
  if (chrome) return 'Chrome ' + chrome[1]; if (fxios) return 'Firefox iOS ' + fxios[1]; if (firefox) return 'Firefox ' + firefox[1]; if (safari) return 'Safari ' + safari[1];
  return securityTrustCurrentDeviceBrowserV366(ua);
}
function diracSecurityMailOsDetailV381(userAgent) {
  const ua = String(userAgent || '').slice(0, 1000);
  const ios = ua.match(/(?:CPU (?:iPhone )?OS|iPhone OS)\s*([0-9_]+)/i), android = ua.match(/Android\s+([0-9.]+)/i), windows = ua.match(/Windows NT\s+([0-9.]+)/i), mac = ua.match(/Mac OS X\s+([0-9_]+)/i);
  if (ios) return 'iOS ' + ios[1].replace(/_/g, '.'); if (android) return 'Android ' + android[1]; if (windows) return 'Windows NT ' + windows[1]; if (mac) return 'macOS ' + mac[1].replace(/_/g, '.');
  return securityTrustCurrentDeviceOsV366(ua);
}
function diracSecurityMailRequestContextV381(req) {
  const headers = req && req.headers && typeof req.headers === 'object' ? req.headers : {};
  const userAgent = String(requestUserAgent(req) || '').trim().slice(0, 1000);
  const publicIpRaw = trustedClientIp(req, process.env), publicIp = publicIpRaw && publicIpRaw !== 'unknown' ? String(publicIpRaw).slice(0, 128) : 'Tidak tersedia';
  ...
  const latitude = vercelTrusted ? safeHeader('x-vercel-ip-latitude', 80) : 'Tidak tersedia';
  const longitude = vercelTrusted ? safeHeader('x-vercel-ip-longitude', 80) : 'Tidak tersedia';
  ...
  const acceptLanguage = safeHeader('accept-language', 300), clientHints = safeHeader('sec-ch-ua', 400), platform = safeHeader('sec-ch-ua-platform', 120), mobileHint = safeHeader('sec-ch-ua-mobile', 40), deviceModel = safeHeader('sec-ch-ua-model', 120);
  const origin = requestOrigin(req) || 'Tidak tersedia', referer = diracSecurityMailSafeRequestUrlV381(headers.referer || headers.referrer), host = safeHeader('host', 253);
  ...
  const browserFingerprint = diracSecurityPasskeyResetSha256V363(JSON.stringify([userAgent, clientHints, platform, mobileHint, acceptLanguage]));
  const deviceFingerprint = diracSecurityPasskeyResetSha256V363(JSON.stringify([securityTrustCurrentDeviceNameV366(userAgent), diracSecurityMailOsDetailV381(userAgent), userAgent, platform]));
  const networkFingerprint = diracSecurityPasskeyResetSha256V363(JSON.stringify([publicIp, asn, networkName, country, region, city]));
  const requestFingerprint = diracSecurityPasskeyResetSha256V363(JSON.stringify([String(req && req.method || ''), String(req && req.url || '').split('?')[0], publicIp, userAgent, origin, host]));
  const signedCookieName = String(process.env.DOMAIN_SIGNED_SESSION_COOKIE || 'dirac_domain_signed_session').trim();
  let sessionFingerprint = 'Tidak tersedia';
  try {
    if (/^[!#$%&'*+\-.^_`|~0-9A-Za-z]{1,128}$/.test(signedCookieName)) {
      const signedCookie = securityResetCentralCookieValueV334(req, signedCookieName);
      if (signedCookie) sessionFingerprint = diracSecurityPasskeyResetSha256V363(signedCookie);
    }
  } catch (_) {}
  return Object.freeze({
    device: securityTrustCurrentDeviceNameV366(userAgent), operatingSystem: diracSecurityMailOsDetailV381(userAgent), browser: diracSecurityMailBrowserDetailV381(userAgent), userAgent: userAgent || 'Tidak tersedia',
    publicIp, localIp: 'Tidak tersedia dari HTTP/browser; alamat IP LAN perangkat tidak diekspos ke server.', asn, networkName,
    country, region, city, timezone, coordinates: latitude !== 'Tidak tersedia' && longitude !== 'Tidak tersedia' ? latitude + ', ' + longitude : 'Tidak tersedia', locationSource: vercelTrusted ? 'Metadata edge Vercel (perkiraan, bukan GPS)' : 'Tidak tersedia',
    acceptLanguage, clientHints, platform, mobileHint, deviceModel: deviceModel === 'Tidak tersedia' ? 'Tidak tersedia dari browser' : deviceModel, browserFingerprint, deviceFingerprint, networkFingerprint, requestFingerprint, sessionFingerprint,
...
function diracSecurityMailClientRowsV381(client) {
  ...
  return [
    ...
    ['IP PUBLIK ENDPOINT/PERANGKAT', c.publicIp], ['IP LOKAL PERANGKAT', c.localIp], ['ASN NETWORK', c.asn], ['NETWORK / ISP', c.networkName],
...
function diracSecuritySocialTextLinesV383() {
  return [
    'X: https://x.com/achzaenuddin?s=11',
...
function diracSecurityCorporateEmailHtmlV327(input = {}) {
  ...
  const rawStatusValue = diracSecurityMailCleanV327(input.statusValue || 'TERLINDUNGI', 180);
  const statusValue = diracSecurityMailEscapeV327(copyCodeExact && (rawStatusValue === copyCodeExact || /^__DIRAC_[A-Z0-9_]+__$/.test(rawStatusValue)) ? 'MENUNGGU KONFIRMASI' : rawStatusValue);
  ...
  const rowsHtml = diracSecurityMailRowsHtmlV327(input.rows, 500, copyCodeExact);
...
function diracSecurityMailTextV327(input = {}) {
  ...
  const rows = (Array.isArray(input.rows) ? input.rows : []).slice(0, 60)
    .map((row) => copyCode && diracSecurityMailCodeExactV382(row && row[1]) === copyCode ? '' : (diracSecurityMailCleanV327(row && row[0], 100) + ': ' + diracSecurityMailCleanV327(row && row[1], 500)));
  ...
  return [
    ...
    '',
    diracSecurityMailCleanV327(input.statusLabel || 'STATUS KEAMANAN', 100) + ': ' + statusValue,
...
async function securityResetSmtpReadV342(socket) {
  return await new Promise((resolve, reject) => {
    ...
    const onData = (chunk) => {
      ...
      if (Buffer.byteLength(buffer, 'utf8') > 65536) return cleanup(reject, resetError('SECURITY_RESET_SMTP_RESPONSE_TOO_LARGE', 503));
...
function securityResetExecutiveReportHtmlV384(value) {
  ...
  const block = '<table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" data-dirac-executive-report="v380" style="width:100%;margin:22px 0 0;border-collapse:collapse"><tr><td align="center" style="padding:0 12px"><table role="presentation" width="600" cellspacing="0" cellpadding="0" border="0" bgcolor="#0b1626" style="width:100%;max-width:600px;border-collapse:separate;border-spacing:0;border:1px solid #28415f;border-radius:18px;overflow:hidden;background:#0b1626;background-color:#0b1626;background-image:linear-gradient(#0b1626,#0b1626)"><tr><td style="height:4px;line-height:4px;font-size:0;background:#19a7c5;background-color:#19a7c5;background-image:linear-gradient(90deg,#19a7c5,#d3a83d)">&nbsp;</td></tr><tr><td style="padding:22px 22px 20px;font-family:Arial,Helvetica,sans-serif;color:#f4f7fb!important;-webkit-text-fill-color:#f4f7fb!important;mso-color-alt:#f4f7fb"><div style="font-size:10px;line-height:1.4;font-weight:800;letter-spacing:.18em;color:#7fd5e8!important;-webkit-text-fill-color:#7fd5e8!important;mso-color-alt:#7fd5e8">JALUR PRIVAT PELANGGAN</div><div style="margin-top:7px;font-size:21px;line-height:1.25;font-weight:800;color:#f4f7fb!important;-webkit-text-fill-color:#f4f7fb!important;mso-color-alt:#f4f7fb">Lapor ke Direktur &amp; Founder</div><p style="margin:10px 0 18px;font-size:13px;line-height:1.65;color:#b8c4d3!important;-webkit-text-fill-color:#b8c4d3!important;mso-color-alt:#b8c4d3">Untuk dugaan penyalahgunaan, penipuan, manipulasi, pemaksaan, atau pelanggaran oleh staf/mitra PT Dirac Inovasi Nusantara, laporan dapat disampaikan langsung kepada Achmad Zaenuddin, Direktur &amp; Founder.</p><table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="width:100%;border-collapse:collapse"><tr><td style="padding:0 0 10px"><a href="mailto:supportdirac@gmail.com?subject=Laporan%20Pelanggan%20ke%20Direktur%20%26%20Founder&amp;body=Halo%20Direktur%20%26%20Founder%20PT%20Dirac%20Inovasi%20Nusantara%2C%0D%0A%0D%0ASaya%20ingin%20menyampaikan%20laporan%20pelanggan.%0D%0A%0D%0ANama%3A%20%0D%0AEmail%20akun%3A%20%0D%0ANomor%20pesanan%2Ftiket%20%28jika%20ada%29%3A%20%0D%0ATanggal%20%26%20waktu%20kejadian%3A%20%0D%0ARingkasan%3A%20%0D%0ABukti%20pendukung%20%28tanpa%20data%20rahasia%29%3A%20%0D%0A%0D%0ATerima%20kasih." style="display:block;padding:13px 16px;border:1px solid #2f91ad;border-radius:12px;background:#12384a;background-color:#12384a;background-image:linear-gradient(#12384a,#12384a);font-size:13px;line-height:1.4;font-weight:800;text-align:center;color:#ffffff!important;-webkit-text-fill-color:#ffffff!important;mso-color-alt:#ffffff;text-decoration:none">✉&nbsp;&nbsp;Siapkan Email Laporan</a></td></tr><tr><td style="padding:0"><a href="https://wa.me/62882009257589?text=Halo%20Direktur%20%26%20Founder%20PT%20Dirac%20Inovasi%20Nusantara%2C%0A%0ASaya%20ingin%20menyampaikan%20laporan%20pelanggan.%0A%0ANama%3A%20%0AEmail%20akun%3A%20%0ANomor%20pesanan%2Ftiket%20%28jika%20ada%29%3A%20%0ATanggal%20%26%20waktu%20kejadian%3A%20%0ARingkasan%3A%20%0ABukti%20pendukung%20%28tanpa%20data%20rahasia%29%3A%20%0A%0ATerima%20kasih." style="display:block;padding:13px 16px;border:1px solid #2f8c62;border-radius:12px;background:#153b2d;background-color:#153b2d;background-image:linear-gradient(#153b2d,#153b2d);font-size:13px;line-height:1.4;font-weight:800;text-align:center;color:#ffffff!important;-webkit-text-fill-color:#ffffff!important;mso-color-alt:#ffffff;text-decoration:none">WhatsApp&nbsp;&nbsp;•&nbsp;&nbsp;+62 882-0092-57589</a></td></tr></table><p style="margin:14px 0 0;font-size:11px;line-height:1.55;color:#8392a6!important;-webkit-text-fill-color:#8392a6!important;mso-color-alt:#8392a6">Template laporan sudah disiapkan otomatis. Lengkapi bagian yang kosong dan jangan sertakan password, OTP, PIN, CVV, passkey, cookie, token, atau secret.</p></td></tr></table></td></tr></table>';
...
async function securityResetHttpMailV352(provider, config, record, subject, text, html, reference) {
  ...
  try {
    ...
    if (!['brevo', 'resend'].includes(provider)) throw resetError('SECURITY_RESET_MAIL_PROVIDER_REJECTED', 503);
    ...
    const body = await securityResetReadUpstreamV361(response, 65536) || {};
...
async function securityResetSendCommittedPasswordMailV342(record, req) {
...
async function securityResetDispatchCommittedMailV338(req) {
  ...
  securityResetMailDiagnosticV340(req, 'dispatch.begin', 'begin', { notice_present: Boolean(notice) });
  if (!notice) return;
  ...
  const delivery = Promise.resolve().then(async () => {
    ...
    const result = await securityResetSendCommittedPasswordMailV342(record, req);
...
async function diracPasswordResetCommitPasswordV333(req, state, password, confirmation, grantId) {
  ...
  const passwordHash = await diracPasswordArgon2V4Hash(cleanPassword, { authUserId, customerId, email: owner.email });
  if (!String(passwordHash).startsWith('$argon2id$')) throw diracPasswordResetErrorV333('PASSWORD_RESET_HASH_PREPARATION_FAILED', 503);
  ...
  if (!ctx) throw resetError('SECURITY_RESET_DB_CONTEXT_REQUIRED', 503);
  ctx.__diracPasswordResetGrantConsumedV361 = true;
  await securityResetAccountAllowedV334(authUserId, owner.email, customerId);
  const currentPasskey = await diracPasswordResetFetchCredentialByRowIdV333(passkeyId, customerId);
  const currentEpoch = await diracPasskeyA2FReadSecurityEpoch(owner);
  if (currentEpoch !== Number(state.security_epoch) || !currentPasskey || currentPasskey.id !== passkeyId
      || currentPasskey.user_id !== customerId || currentPasskey.credential_id !== credentialId
      || currentPasskey.is_active !== true || currentPasskey.rotation_state !== 'active') {
    throw diracPasswordResetErrorV333('PASSWORD_RESET_AUTHORIZATION_CHANGED', 409);
  }
  ...
  const shadow = await diracPasswordArgon2ActiveOnlyV120UpsertCurrent(authUserId, activeOnlyRow, nowIso);
...
/* ============================================================
   DIRAC SECURITY PASSKEY RESET V364
   ...
   active signed session/MFA epoch + current account password + one-time email
   verification code. The current Passkey is never asserted for authorization.
   ============================================================ */
...
const DIRAC_SECURITY_PASSKEY_RESET_EMAIL_TTL_MS_V364 = 120000;
const DIRAC_SECURITY_PASSKEY_RESET_EMAIL_CODE_MIN_LENGTH_V364 = 512;
const DIRAC_SECURITY_PASSKEY_RESET_EMAIL_CODE_MAX_LENGTH_V364 = 768;
const DIRAC_SECURITY_PASSKEY_RESET_EMAIL_SYMBOLS_V374 = '!#$%&()*+,-./:;<=>?@[]^{|}~';
const DIRAC_SECURITY_PASSKEY_RESET_EMAIL_ALPHABET_V374 = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789_' + DIRAC_SECURITY_PASSKEY_RESET_EMAIL_SYMBOLS_V374;
...
async function diracSecurityPasskeyResetResolveOwnerV363(req) {
  ...
  return Object.freeze({ owner, sessionId: session.sessionId, securityEpoch, mfaExpiresAt: session.mfaExpiresAt, signedIssuedAt: session.issuedAt, signedExpiresAt: session.expiresAt });
}
...
function diracSecurityPasskeyResetAuthSessionIdV371(accessToken, owner) {
  const token=String(accessToken||'').trim(),parts=token.split('.');
  if(!owner||parts.length!==3||!parts[0]||!parts[1]||!parts[2]||token.length>20*1024||!/^[A-Za-z0-9_-]+$/.test(parts[0])||!/^[A-Za-z0-9_-]+$/.test(parts[1])||!/^[A-Za-z0-9_-]+$/.test(parts[2]))throw resetError('SECURITY_PASSKEY_RESET_PASSWORD_PROVIDER_CONTRACT_INVALID',503);
  let raw=Buffer.alloc(0);
  try{
    raw=Buffer.from(parts[1],'base64url');if(!raw.length||raw.length>8192||raw.toString('base64url')!==parts[1])throw resetError('SECURITY_PASSKEY_RESET_PASSWORD_PROVIDER_CONTRACT_INVALID',503);
    const payload=parseStrictJson(raw,{maxBytes:8192,maxDepth:12,maxNodes:512}),now=Math.floor(Date.now()/1000),sessionId=String(payload&&payload.session_id||'');
    if(!payload||typeof payload!=='object'||Array.isArray(payload)||!customerSecurityLooksLikeUuid(sessionId)||!safeEqual(String(payload.sub||''),owner.authUserId)||String(payload.aud||'')!=='authenticated'||String(payload.role||'')!=='authenticated'||!Number.isSafeInteger(Number(payload.iat))||Number(payload.iat)>now+30||!Number.isSafeInteger(Number(payload.exp))||Number(payload.exp)<=now+30||(payload.email!==undefined&&!safeEqual(normalizeAuthEmail(payload.email||''),owner.email)))throw resetError('SECURITY_PASSKEY_RESET_PASSWORD_PROVIDER_CONTRACT_INVALID',503);
    return sessionId;
  }catch(error){if(error&&error.code)throw error;throw resetError('SECURITY_PASSKEY_RESET_PASSWORD_PROVIDER_CONTRACT_INVALID',503);}
  finally{if(raw.length)raw.fill(0);}
}

async function diracSecurityPasskeyReplacementVerifyPasswordV367(owner, password) {
  const value = String(password === undefined || password === null ? '' : password).normalize('NFC');
  if (!value || value.length > 128 || Buffer.byteLength(value, 'utf8') > 512 || /[\u0000\r\n]/.test(value)) throw resetError('SECURITY_PASSKEY_RESET_PASSWORD_INVALID', 422);
  const ctx = diracCentralCurrentContextV149();
  if (!ctx || !ctx.active || !ctx.__diracPasswordResetVerifiedOwnerV333) throw resetError('SECURITY_PASSKEY_RESET_PASSWORD_CONTEXT_INVALID', 503);
  ctx.__diracPasskeyResetPasswordCheckV363 = true;
  try {
    ...
    const accessToken = String(verified.data && verified.data.access_token || '');
    if (!user || !accessToken) throw resetError('SECURITY_PASSKEY_RESET_PASSWORD_PROVIDER_CONTRACT_INVALID', 503);
    if (!safeEqual(String(user.id || ''), owner.authUserId) || !safeEqual(normalizeAuthEmail(user.email || ''), owner.email)) throw resetError('SECURITY_PASSKEY_RESET_PASSWORD_IDENTITY_MISMATCH', 403);
    return Object.freeze({rpcAuthSessionId:diracSecurityPasskeyResetAuthSessionIdV371(accessToken,owner),accessToken});
  } finally {
    ctx.__diracPasskeyResetPasswordCheckV363 = false;
    ctx.__diracPasskeyResetEphemeralAccessDigestV363 = '';
  }
...
async function diracSecurityPasskeyResetCleanupAuthSessionV371(accessToken) {
  const token=String(accessToken||'').trim();if(!token)return false;
  const ctx=diracCentralCurrentContextV149();if(!ctx||!ctx.active||!ctx.__diracPasswordResetVerifiedOwnerV333)return false;
  ctx.__diracPasskeyResetPasswordCheckV363=true;ctx.__diracPasskeyResetEphemeralAccessDigestV363=diracSecurityPasskeyResetSha256V363(token);
  try{const logout=await supabaseFetch('/auth/v1/logout?scope=local',{method:'POST',auth:'anon',bearer:token});return Boolean(logout&&logout.ok===true);}
  finally{ctx.__diracPasskeyResetPasswordCheckV363=false;ctx.__diracPasskeyResetEphemeralAccessDigestV363='';}
}
...
function diracSecurityPasskeyResetEmailCodeV364() {
  ...
  const mandatory = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ'[crypto.randomInt(0, 26)]
    ...
    + '_'
    + DIRAC_SECURITY_PASSKEY_RESET_EMAIL_SYMBOLS_V374[crypto.randomInt(0, DIRAC_SECURITY_PASSKEY_RESET_EMAIL_SYMBOLS_V374.length)];
  return (mandatory + crypto.randomBytes(length).toString('base64url')).slice(0, length);
}

function diracSecurityPasskeyResetEmailCodeLooksValidV374(code, expectedLength) {
  const value = String(code || '');
  return Number.isSafeInteger(Number(expectedLength))
    && Number(expectedLength) >= DIRAC_SECURITY_PASSKEY_RESET_EMAIL_CODE_MIN_LENGTH_V364
    && Number(expectedLength) <= DIRAC_SECURITY_PASSKEY_RESET_EMAIL_CODE_MAX_LENGTH_V364
...
function diracSecurityPasskeyResetEmailCodeMacV364(payload, code) {
  const key = diracCentralDeriveSecretV146('security-passkey-reset-email-code-v364');
  try {
...
function diracSecurityPasskeyResetEmailTokenV364(req, context, code) {
  const now = Date.now();
  const base = {
    typ: DIRAC_SECURITY_PASSKEY_RESET_EMAIL_V364,
    phase: 'email',
    jti: randomToken(24),
    ...
    sessionId: String(context.sessionId || ''), rpcAuthSessionId: String(context.rpcAuthSessionId || ''), securityEpoch: Number(context.securityEpoch || 0),
    ...
    serverDeviceBindingKeyId: String(context.serverDeviceBindingKeyId || ''),
...
async function diracSecurityPasskeyResetReadEmailTokenV364(req, token, resolved) {
  const raw = String(token || '').trim();
  if (raw.length < 48 || raw.length > 4096) throw resetError('SECURITY_PASSKEY_RESET_EMAIL_TOKEN_INVALID', 403);
  const p = securityResetTokenDecodeV334(raw, DIRAC_SECURITY_PASSKEY_RESET_EMAIL_TOKEN_SCOPE_V364);
  const now = Date.now();
  if (!p || p.typ !== DIRAC_SECURITY_PASSKEY_RESET_EMAIL_V364 || p.phase !== 'email'
      || !/^[A-Za-z0-9_-]{32}$/.test(String(p.jti || '')) || !/^[A-Za-z0-9_-]{43}$/.test(String(p.challenge || '')) || !/^[A-Za-z0-9_-]{24}$/.test(String(p.codeSalt || '')) || !/^[a-f0-9]{64}$/.test(String(p.codeMac || ''))
      || !customerSecurityLooksLikeUuid(String(p.authUserId || '')) || !customerSecurityLooksLikeUuid(String(p.customerId || '')) || !customerSecurityLooksLikeUuid(String(p.sessionId || '')) || !customerSecurityLooksLikeUuid(String(p.rpcAuthSessionId || ''))
      ...
      || !/^[a-f0-9]{64}$/.test(String(p.serverDeviceBindingKeyId || ''))
      ...
      || Number(p.expiresAtMs) - Number(p.issuedAtMs) !== DIRAC_SECURITY_PASSKEY_RESET_EMAIL_TTL_MS_V364
      ...
      || !safeEqual(String(p.sessionId), resolved.sessionId) || Number(p.securityEpoch) !== resolved.securityEpoch) throw resetError('SECURITY_PASSKEY_RESET_EMAIL_TOKEN_INVALID', 403);
  const currentDevice = diracSecurityPasskeyResetServerDeviceAuthorityV367().read(req, resolved.owner);
  if (!currentDevice || currentDevice.ok !== true || !safeEqual(String(currentDevice.keyId || ''), String(p.serverDeviceBindingKeyId || ''))) throw resetError('SECURITY_PASSKEY_RESET_DEVICE_BINDING_CHANGED', 403);
  return Object.freeze({ token: raw, payload: p });
}

const DIRAC_SECURITY_SMTP_REQUEST_RATE_V374 = 'dirac-security-smtp-request-rate-v374';
const DIRAC_SECURITY_SMTP_REQUEST_RATE_PERMANENT_UNTIL_MS_V374 = 253370764800000;

function diracSecuritySmtpRequestRateFlowV374(label) {
  const clean = String(label || '').trim().toLowerCase();
  if (clean === 'send' || clean === 'success') return 'passkey_reset';
  if (clean === 'password_change_send' || clean === 'password_change_success') return 'password_change';
  return '';
}
...
function diracSecuritySmtpRequestRateKeyV374(flow, owner) {
  return 'security-smtp-request-rate-v374:' + diracCentralHashV146([DIRAC_SECURITY_SMTP_REQUEST_RATE_V374, String(flow || ''), String(owner && owner.authUserId || ''), String(owner && owner.customerId || '')].join('|'));
}

async function diracSecuritySmtpRequestRateWriteV374(req, flow, owner, attemptCount, blockedUntilMs) {
  const context = diracCentralCurrentContextV149();
  const key = diracSecuritySmtpRequestRateKeyV374(flow, owner);
  const now = Date.now();
  const record = { type: DIRAC_SECURITY_SMTP_REQUEST_RATE_V374, flow, auth_user_id: owner.authUserId, customer_id: owner.customerId, attempt_count: attemptCount, blocked_until_ms: blockedUntilMs, updated_at_ms: now };
  context.__diracSecuritySmtpRequestRateV374 = Object.freeze({ active: true, key, flow, authUserId: owner.authUserId, customerId: owner.customerId, attemptCount, blockedUntilMs });
  let result;
  try {
    result = await supabaseFetch('/rest/v1/dirac_persistent_bans?on_conflict=security_key', { method: 'POST', auth: 'service', prefer: 'resolution=merge-duplicates,return=representation', body: [{ security_key: key, record_json: record, blocked_until_ms: blockedUntilMs, updated_at: new Date(now).toISOString(), expires_at: '9999-01-01T00:00:00.000Z' }] });
  } finally { context.__diracSecuritySmtpRequestRateV374 = null; }
  const row = result && result.ok === true && Array.isArray(result.data) && result.data.length === 1 ? result.data[0] : null;
  if (!row || String(row.security_key || '') !== key || Number(row.blocked_until_ms || 0) !== blockedUntilMs) throw resetError('SECURITY_SMTP_REQUEST_RATE_STORAGE_UNAVAILABLE', 503);
  return true;
}

async function diracSecuritySmtpRequestRateTakeV374(req, flow, owner) {
  const key = diracSecuritySmtpRequestRateKeyV374(flow, owner);
  const result = await supabaseFetch('/rest/v1/dirac_persistent_bans?select=' + encodeURIComponent('security_key,record_json,blocked_until_ms,expires_at') + '&security_key=eq.' + encodeURIComponent(key) + '&limit=1', { method: 'GET', auth: 'service' });
  if (!result || result.ok !== true || !Array.isArray(result.data) || result.data.length > 1) throw resetError('SECURITY_SMTP_REQUEST_RATE_STORAGE_UNAVAILABLE', 503);
  ...
  if (row && (!record || !exactKeys(record, ['type','flow','auth_user_id','customer_id','attempt_count','blocked_until_ms','updated_at_ms'])
      || record.type !== DIRAC_SECURITY_SMTP_REQUEST_RATE_V374 || record.flow !== flow
      || !safeEqual(String(record.auth_user_id || ''), owner.authUserId) || !safeEqual(String(record.customer_id || ''), owner.customerId)
      || !Number.isSafeInteger(Number(record.attempt_count)) || Number(record.attempt_count) < 0 || Number(record.attempt_count) > 5
...
async function diracSecuritySmtpRequestRateResetV374(req, flow, owner) {
...
async function diracSecurityPasskeyResetEmailRateV364(req, label, identity, limit, windowSeconds, blockSeconds) {
  ...
  const ip = trustedClientIp(req, process.env);
  const key = 's2s-central-rate-v230:' + diracCentralHashV146(['keamanan-passkey-email-v364', String(label || ''), String(identity || ''), ip].join('|'));
  const result = await supabaseFetch('/rest/v1/rpc/dirac_central_atomic_rate_limit_v230', { method: 'POST', auth: 'service', body: { p_security_key: key, p_limit: limit, p_window_seconds: windowSeconds, p_block_seconds: blockSeconds } }).catch(() => null);
  const row = result && result.ok === true && Array.isArray(result.data) ? result.data[0] : null;
  if (!row || typeof row.allowed !== 'boolean') throw resetError('SECURITY_PASSKEY_RESET_EMAIL_RATE_UNAVAILABLE', 503);
  if (row.allowed !== true) throw resetError('SECURITY_PASSKEY_RESET_EMAIL_RATE_LIMITED', 429);
  return true;
}

async function diracSecurityPasskeyResetMailCascadeV372(config, record, subject, text, html, reference) {
  let result = await securityResetHttpMailV352('brevo', config, record, subject, text, html, reference);
  if (!result || result.ok !== true) result = await securityResetHttpMailV352('resend', config, record, subject, text, html, reference);
  if (!result || result.ok !== true) result = await securityResetGmailMailV352(config.primary, record, subject, text, html, 1);
  return result;
}

async function diracSecurityPasskeyResetSendEmailCodeV364(req, owner, code, payload) {
  const config = securityResetUserMailConfigV352();
  ...
  if (!context || !context.active || !context.__diracPasswordResetVerifiedOwnerV333
      ...
      || !safeEqual(String(context.__diracPasswordResetVerifiedOwnerV333.customerId || ''), owner.customerId)) throw resetError('SECURITY_PASSKEY_RESET_EMAIL_CONTEXT_INVALID', 503);
  context.__diracPasskeyResetEmailDispatchV364 = true;
  context.__diracPasskeyResetEmailRecipientHashV364 = diracSecurityPasskeyResetSha256V363(owner.email);
  let result;
  try {
    result = await diracSecurityPasskeyResetMailCascadeV372(config, record, subject, text, html, reference);
  } finally {
    context.__diracPasskeyResetEmailDispatchV364 = false;
    context.__diracPasskeyResetEmailRecipientHashV364 = '';
  }
  securityResetMailDiagnosticV340(req, 'passkey_reset.email_code', result && result.ok === true ? 'success' : 'error', { delivered: Boolean(result && result.ok === true), provider: String(result && result.provider || ''), upstream_status: Number(result && result.status || 0), reference });
  if (!result || result.ok !== true) throw resetError('SECURITY_PASSKEY_RESET_EMAIL_DELIVERY_FAILED', 503);
  return true;
}

async function diracSecurityPasskeyResetSendChangedNoticeV372(req, owner, rotationId, securityEpoch) {
  const config = securityResetUserMailConfigV352();
  if (!config) return Object.freeze({ ok: false, provider: '', status: 0, code: 'SECURITY_PASSKEY_RESET_NOTICE_CONFIGURATION_UNAVAILABLE' });
  ...
  const htmlInput = {
    ...
    detailsLabel: 'DETAIL PERUBAHAN', rows: [['AKTIVITAS','Penggantian Passkey'],['WAKTU',timeText],['SECURITY EPOCH',String(securityEpoch)], ...diracSecurityMailClientRowsV381(client), ['REFERENSI',reference]],
    warningTitle: 'PERIKSA JIKA BUKAN ANDA', warning: 'Jika Anda tidak melakukan perubahan ini, segera tinjau keamanan akun dan hubungi kanal resmi. Jangan bagikan password, kode verifikasi, token, cookie, atau Passkey.',
    actionUrl: diracRoleOriginV250('security') + '/keamanan.html', actionText: 'BUKA PUSAT KEAMANAN'
  };
  const text = diracSecurityMailTextV327(htmlInput);
  const html = diracSecurityCorporateEmailHtmlV327(htmlInput);
  ...
  if (!context || !context.active || !context.__diracPasswordResetVerifiedOwnerV333
      || !safeEqual(String(context.__diracPasswordResetVerifiedOwnerV333.authUserId || ''), owner.authUserId)
      || !safeEqual(String(context.__diracPasswordResetVerifiedOwnerV333.customerId || ''), owner.customerId)) return Object.freeze({ ok: false, provider: '', status: 0, code: 'SECURITY_PASSKEY_RESET_NOTICE_CONTEXT_INVALID' });
  context.__diracPasskeyResetEmailDispatchV364 = true;
  context.__diracPasskeyResetEmailRecipientHashV364 = diracSecurityPasskeyResetSha256V363(owner.email);
  let result;
  try {
  ...
  securityResetMailDiagnosticV340(req, 'passkey_reset.changed_notice', result && result.ok === true ? 'success' : 'error', { delivered: Boolean(result && result.ok === true), provider: String(result && result.provider || ''), upstream_status: Number(result && result.status || 0), reference });
  return Object.freeze({ ok: Boolean(result && result.ok === true), provider: String(result && result.provider || ''), status: Number(result && result.status || 0), code: String(result && result.code || '') });
}
...
const DIRAC_PASSWORD_CHANGE_SMTP_TTL_MS_V365 = 120000;
const DIRAC_PASSWORD_CHANGE_SMTP_CODE_MIN_LENGTH_V365 = 512;
const DIRAC_PASSWORD_CHANGE_SMTP_CODE_MAX_LENGTH_V365 = 768;
const DIRAC_PASSWORD_CHANGE_SMTP_SYMBOLS_V365 = '!#$%&()*+,-./:;<=>?@[]^{|}~';
const DIRAC_PASSWORD_CHANGE_SMTP_ALPHABET_V365 = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789_' + DIRAC_PASSWORD_CHANGE_SMTP_SYMBOLS_V365;
...
function diracPasswordChangeSmtpCodeV365() {
  ...
  const mandatory = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ'[crypto.randomInt(0, 26)]
    ...
    + '0123456789'[crypto.randomInt(0, 10)]
    + '_'
    + DIRAC_PASSWORD_CHANGE_SMTP_SYMBOLS_V365[crypto.randomInt(0, DIRAC_PASSWORD_CHANGE_SMTP_SYMBOLS_V365.length)];
  const code = (mandatory + crypto.randomBytes(length).toString('base64url')).slice(0, length);
  if (!diracPasswordChangeSmtpCodeLooksValidV365(code, length)) throw resetError('PASSWORD_CHANGE_SMTP_CODE_GENERATION_INVALID', 500);
  return code;
}
...
async function diracSecurityPasskeyResetConsumeEmailTokenV364(token, payload) {
  const result = await diracCentralAtomicConsumeV230({
    ...
    contextHash: [DIRAC_SECURITY_PASSKEY_RESET_EMAIL_V364, payload.jti, payload.authUserId, payload.customerId, payload.sessionId, String(payload.securityEpoch), payload.authorizingCredentialIdHash, diracSecurityPasskeyResetSha256V363(token)].join('|')
  });
  if (!result || result.ok !== true) throw resetError('SECURITY_PASSKEY_RESET_EMAIL_TOKEN_REPLAY', 409);
  return true;
}
...
function diracSecurityPasskeyResetTokenV363(req, phase, context) {
  ...
  const payload = Object.freeze({
    ...
    sessionId: String(context.sessionId || ''), rpcAuthSessionId: String(context.rpcAuthSessionId || ''), securityEpoch: Number(context.securityEpoch || 0),
    authorizingPasskeyId: String(context.authorizingPasskeyId || ''), authorizingCredentialIdHash: String(context.authorizingCredentialIdHash || ''), activeCredentialSetHash: String(context.activeCredentialSetHash || ''),
    authorizationMethod: String(context.authorizationMethod || ''), emailVerificationHash: String(context.emailVerificationHash || ''),
    serverDeviceBindingKeyId: String(context.serverDeviceBindingKeyId || ''),
...
async function diracSecurityPasskeyResetReadTokenV363(req, token, expectedPhase, resolved) {
  ...
  if ((expectedPhase === 'authorize' || expectedPhase === 'register' || expectedPhase === 'confirm')
      && (p.authorizationMethod !== 'password_email' || !/^[a-f0-9]{64}$/.test(String(p.emailVerificationHash || '')))) {
    throw resetError('SECURITY_PASSKEY_RESET_EMAIL_AUTHORIZATION_REQUIRED', 403);
  }
  const currentDevice = diracSecurityPasskeyResetServerDeviceAuthorityV367().read(req, resolved.owner);
  if (!currentDevice || currentDevice.ok !== true || !safeEqual(String(currentDevice.keyId || ''), String(p.serverDeviceBindingKeyId || ''))) throw resetError('SECURITY_PASSKEY_RESET_DEVICE_BINDING_CHANGED', 403);
  if ((expectedPhase === 'authorize' || expectedPhase === 'register') && !customerSecurityLooksLikeUuid(String(p.rotationId || ''))) throw resetError('SECURITY_PASSKEY_RESET_ROTATION_TOKEN_INVALID', 403);
...
async function diracSecurityPasskeyResetConsumeTokenV363(token, payload) {
  const result = await diracCentralAtomicConsumeV230({
    ...
    contextHash: [DIRAC_SECURITY_PASSKEY_RESET_V363, payload.phase, payload.jti, payload.authUserId, payload.customerId, payload.sessionId, payload.rpcAuthSessionId, String(payload.securityEpoch), payload.challenge, diracSecurityPasskeyResetSha256V363(token)].join('|')
  });
...
function diracSecurityPasskeyResetStoredServerDeviceBindingV367(row){
  const j=row&&row.credential_json&&typeof row.credential_json==='object'?row.credential_json:{},b=j.device_binding&&typeof j.device_binding==='object'?j.device_binding:null;
  if(!b||b.required!==true||b.version!==DIRAC_PASSKEY_SERVER_DEVICE_BINDING_VERSION_V367||b.algorithm!==DIRAC_PASSKEY_SERVER_DEVICE_BINDING_ALGORITHM_V367||b.policy!==DIRAC_PASSKEY_SERVER_DEVICE_BINDING_POLICY_V367||!/^[a-f0-9]{64}$/.test(String(b.key_id||'')))return{ok:false,reason:'server_device_binding_stored_policy_invalid'};
  return{ok:true,keyId:String(b.key_id),binding:b,version:DIRAC_PASSKEY_SERVER_DEVICE_BINDING_VERSION_V367,algorithm:DIRAC_PASSKEY_SERVER_DEVICE_BINDING_ALGORITHM_V367,policy:DIRAC_PASSKEY_SERVER_DEVICE_BINDING_POLICY_V367,serverBound:true};
}

function diracSecurityPasskeyResetRegistrationDeviceBindingV363(req,owner,body,payload){
  if(diracPasskeyA2FSafeString(body&&body.response,64)!=='dpk2s')return{ok:false,reason:'server_device_binding_marker_invalid'};
  const expected=String(payload&&payload.serverDeviceBindingKeyId||'');if(!/^[a-f0-9]{64}$/.test(expected))return{ok:false,reason:'server_device_binding_challenge_invalid'};
  const current=diracSecurityPasskeyResetServerDeviceAuthorityV367().read(req,owner);if(!current||current.ok!==true)return{ok:false,reason:String(current&&current.reason||'server_device_binding_cookie_invalid')};
  if(!safeEqual(String(current.keyId||''),expected))return{ok:false,reason:'server_device_binding_key_mismatch'};
  return{ok:true,keyId:current.keyId,version:DIRAC_PASSKEY_SERVER_DEVICE_BINDING_VERSION_V367,algorithm:DIRAC_PASSKEY_SERVER_DEVICE_BINDING_ALGORITHM_V367,policy:DIRAC_PASSKEY_SERVER_DEVICE_BINDING_POLICY_V367,serverBound:true};
}
...
function diracSecurityPasskeyResetAuthenticationDeviceBindingV363(req,owner,row,body,payload){
  const stored=diracSecurityPasskeyResetStoredServerDeviceBindingV367(row);if(!stored.ok)return stored;
  if(diracPasskeyA2FSafeString(body&&body.response,64)!=='dpk2s')return{ok:false,reason:'server_device_binding_marker_invalid'};
  const expected=String(payload&&payload.serverDeviceBindingKeyId||'');if(!/^[a-f0-9]{64}$/.test(expected))return{ok:false,reason:'server_device_binding_challenge_invalid'};
  const current=diracSecurityPasskeyResetServerDeviceAuthorityV367().read(req,owner);if(!current||current.ok!==true)return{ok:false,reason:String(current&&current.reason||'server_device_binding_cookie_invalid')};
  if(!safeEqual(String(current.keyId||''),expected)||!safeEqual(stored.keyId,current.keyId))return{ok:false,reason:'server_device_binding_key_mismatch'};
  return{ok:true,keyId:current.keyId,binding:stored.binding,version:DIRAC_PASSKEY_SERVER_DEVICE_BINDING_VERSION_V367,algorithm:DIRAC_PASSKEY_SERVER_DEVICE_BINDING_ALGORITHM_V367,policy:DIRAC_PASSKEY_SERVER_DEVICE_BINDING_POLICY_V367,serverBound:true};
}
...
async function diracSecurityPasskeyResetRecordAssertionV363({req,row,owner,payload,credential,body,setupToken,purpose,confirmPending,rotationId}) {
  ...
  const device=diracSecurityPasskeyResetAuthenticationDeviceBindingV363(req,owner,row,body,payload);if(!device.ok)throw resetError('SECURITY_PASSKEY_RESET_DEVICE_BINDING_INVALID',403);
...
async function diracSecurityPasskeyResetStartV363(req,res,body) {
  ...
  try{
    ...
    const row=active[0],credentialHash=diracSecurityPasskeyResetSha256V363(String(row.credential_id)),activeSetHash=diracSecurityPasskeyResetActiveSetHashV363(active);
    await diracSecurityPasskeyResetEmailRateV364(req,'send',resolved.owner.customerId,3,600,600);
    const code=diracSecurityPasskeyResetEmailCodeV364();
    const issued=diracSecurityPasskeyResetEmailTokenV364(req,{authUserId:resolved.owner.authUserId,customerId:resolved.owner.customerId,email:resolved.owner.email,sessionId:resolved.sessionId,rpcAuthSessionId:passwordAuth.rpcAuthSessionId,securityEpoch:resolved.securityEpoch,authorizingPasskeyId:String(row.id),authorizingCredentialIdHash:credentialHash,activeCredentialSetHash:activeSetHash,serverDeviceBindingKeyId:serverDevice.keyId},code);
    await diracSecurityPasskeyResetSendEmailCodeV364(req,resolved.owner,code,issued.payload);retainAuthSession=true;
  ...
  }finally{if(!retainAuthSession)await diracSecurityPasskeyResetCleanupAuthSessionV371(passwordAuth.accessToken).catch(()=>false);}
}
...
async function diracSecurityPasskeyResetEmailAuthorizeV364(req,body,resolved,tokenData) {
  ...
  if(!diracSecurityPasskeyResetEmailCodeLooksValidV374(code,codeLength))throw resetError('SECURITY_PASSKEY_RESET_EMAIL_CODE_INVALID',403);
  ...
  const active=await diracSecurityPasskeyResetListActiveV363(resolved.owner);if(active.length!==1||!diracSecurityPasskeyResetValidateOwnerRowV363(active[0],resolved.owner)||!safeEqual(String(active[0].id||''),p.authorizingPasskeyId)||!safeEqual(diracSecurityPasskeyResetSha256V363(active[0].credential_id),p.authorizingCredentialIdHash)||!safeEqual(diracSecurityPasskeyResetActiveSetHashV363(active),p.activeCredentialSetHash)||!customerSecurityLooksLikeUuid(String(p.rpcAuthSessionId||'')))throw resetError('SECURITY_PASSKEY_RESET_ACTIVE_SET_CHANGED',409);
  await diracSecurityPasskeyResetConsumeEmailTokenV364(tokenData.token,p);
  await diracSecurityPasskeyResetEmailRateV364(req,'success',resolved.owner.customerId,1,1,1);
  const emailVerificationHash=diracSecurityPasskeyResetSha256V363([p.jti,p.codeMac,p.sessionId,String(p.securityEpoch)].join('|')),rotationId=crypto.randomUUID();
  const issued=diracSecurityPasskeyResetTokenV363(req,'authorize',{authUserId:resolved.owner.authUserId,customerId:resolved.owner.customerId,email:resolved.owner.email,sessionId:resolved.sessionId,rpcAuthSessionId:p.rpcAuthSessionId,securityEpoch:resolved.securityEpoch,authorizingPasskeyId:p.authorizingPasskeyId,authorizingCredentialIdHash:p.authorizingCredentialIdHash,activeCredentialSetHash:p.activeCredentialSetHash,authorizationMethod:'password_email',emailVerificationHash,serverDeviceBindingKeyId:p.serverDeviceBindingKeyId,rotationId});
  return{ok:true,method:'passkey',passkeyMode:'authorize_old',reset_passkey:true,authorized:true,authorization_method:'password_email',email_verified:true,old_passkey_authorization_required:true,device_binding_required:true,device_binding_policy:DIRAC_PASSKEY_SERVER_DEVICE_BINDING_POLICY_V367,setupToken:issued.token,mfaSetupToken:issued.token,expires_in:Math.floor((issued.payload.expiresAtMs-Date.now())/1000),publicKey:{challenge:issued.payload.challenge,rpId:issued.payload.rpId,timeout:Math.min(60000,issued.payload.expiresAtMs-Date.now()),userVerification:'required',allowCredentials:[{type:'public-key',id:String(active[0].credential_id),transports:Array.isArray(active[0].transports)?active[0].transports:[]}]},message:'Kode email terverifikasi. Konfirmasikan Passkey lama untuk mengotorisasi penggantian.'};
}

async function diracSecurityPasskeyResetAuthorizeOldV371(req,body,resolved,tokenData) {
  const p=tokenData.payload,active=await diracSecurityPasskeyResetListActiveV363(resolved.owner);if(active.length!==1||!customerSecurityLooksLikeUuid(String(p.rotationId||''))||!diracSecurityPasskeyResetValidateOwnerRowV363(active[0],resolved.owner)||!safeEqual(String(active[0].id||''),p.authorizingPasskeyId)||!safeEqual(diracSecurityPasskeyResetSha256V363(active[0].credential_id),p.authorizingCredentialIdHash)||!safeEqual(diracSecurityPasskeyResetActiveSetHashV363(active),p.activeCredentialSetHash))throw resetError('SECURITY_PASSKEY_RESET_ACTIVE_SET_CHANGED',409);
  const recorded=await diracSecurityPasskeyResetRecordAssertionV363({req,row:active[0],owner:resolved.owner,payload:p,credential:body.credential,body,setupToken:tokenData.token,purpose:'replace_authorize_old',confirmPending:false,rotationId:p.rotationId}),authorizedHash=String(recorded&&recorded.rpcData&&recorded.rpcData.authorizing_credential_id_hash||'');
  if(!/^[a-f0-9]{64}$/.test(authorizedHash)||!safeEqual(authorizedHash,p.authorizingCredentialIdHash))throw resetError('SECURITY_PASSKEY_RESET_OLD_PASSKEY_AUTHORIZATION_FAILED',503);
  const refreshed=await diracSecurityPasskeyResetListActiveV363(resolved.owner);if(refreshed.length!==1||!diracSecurityPasskeyResetValidateOwnerRowV363(refreshed[0],resolved.owner)||!safeEqual(String(refreshed[0].id||''),p.authorizingPasskeyId)||Number(refreshed[0].sign_count||0)!==recorded.nextSignCount||!safeEqual(diracSecurityPasskeyResetActiveSetHashV363(refreshed),p.activeCredentialSetHash))throw resetError('SECURITY_PASSKEY_RESET_OLD_PASSKEY_POSTCONDITION_FAILED',503);
  const issued=diracSecurityPasskeyResetTokenV363(req,'register',{authUserId:resolved.owner.authUserId,customerId:resolved.owner.customerId,email:resolved.owner.email,sessionId:resolved.sessionId,rpcAuthSessionId:p.rpcAuthSessionId,securityEpoch:resolved.securityEpoch,authorizingPasskeyId:p.authorizingPasskeyId,authorizingCredentialIdHash:authorizedHash,activeCredentialSetHash:p.activeCredentialSetHash,authorizationMethod:p.authorizationMethod,emailVerificationHash:p.emailVerificationHash,serverDeviceBindingKeyId:p.serverDeviceBindingKeyId,rotationId:p.rotationId});
  const userHandle=crypto.createHash('sha256').update('dirac-passkey-user:'+resolved.owner.customerId).digest('base64url');
  return{ok:true,method:'passkey',passkeyMode:'registration',reset_passkey:true,authorized:true,authorization_method:'password_email',email_verified:true,old_passkey_verified:true,needsRegistration:true,device_binding_required:true,device_binding_policy:DIRAC_PASSKEY_SERVER_DEVICE_BINDING_POLICY_V367,setupToken:issued.token,mfaSetupToken:issued.token,expires_in:Math.floor((issued.payload.expiresAtMs-Date.now())/1000),publicKey:{challenge:issued.payload.challenge,rp:{name:String(process.env.WEBAUTHN_RP_NAME||process.env.DIRAC_WEBAUTHN_RP_NAME||'PT Dirac Inovasi Nusantara').trim()||'PT Dirac Inovasi Nusantara',id:issued.payload.rpId},user:{id:userHandle,name:resolved.owner.email,displayName:resolved.owner.email},pubKeyCredParams:[{type:'public-key',alg:-7},{type:'public-key',alg:-257}],timeout:60000,attestation:'none',authenticatorSelection:{authenticatorAttachment:'platform',residentKey:'preferred',requireResidentKey:false,userVerification:'required'},extensions:{credProps:true},excludeCredentials:[]},message:'Passkey lama terverifikasi. Buat Passkey pengganti baru.'};
}
...
async function diracSecurityPasskeyResetRegisterV363(req,body,resolved,tokenData) {
  ...
  const device=diracSecurityPasskeyResetRegistrationDeviceBindingV363(req,resolved.owner,body,p);if(!device.ok)throw resetError('SECURITY_PASSKEY_RESET_DEVICE_BINDING_INVALID',403);
  ...
  const nowIso=new Date().toISOString(),credentialJson=diracSecurityPasskeyResetMinimalCredentialJsonV363({credential,response,clientData,payload:p,owner:resolved.owner,mode:'registration'});credentialJson.webauthn={rp_id:p.rpId,public_key_jwk:registration.publicKeyJwk,credential_public_key_cose:registration.credentialPublicKeyCose,aaguid:registration.aaguid,rotation_version:'dirac-passkey-single-active-rotation-v237',sign_count:signCount,backup_eligible:registration.backupEligible===true,backup_state:registration.backupState===true,device_bound:registration.deviceBound===true,sync_policy:'synced-passkey-server-device-binding-required-v2',verified_at:nowIso};credentialJson.device_binding={version:DIRAC_PASSKEY_SERVER_DEVICE_BINDING_VERSION_V367,algorithm:DIRAC_PASSKEY_SERVER_DEVICE_BINDING_ALGORITHM_V367,policy:DIRAC_PASSKEY_SERVER_DEVICE_BINDING_POLICY_V367,required:true,key_id:device.keyId,registered_at:nowIso,last_verified_at:nowIso};credentialJson.security_passkey_reset_v363={authorized_credential_id_hash:p.authorizingCredentialIdHash,authorized_session_id:p.rpcAuthSessionId,authorized_at:new Date(Number(p.issuedAtMs)).toISOString()};
  const rotationId=p.rotationId,pendingExpiresAtMs=Date.now()+DIRAC_SECURITY_PASSKEY_RESET_PENDING_TTL_MS_V363;
  ...
  try{
    ...
    const issued=diracSecurityPasskeyResetTokenV363(req,'confirm',{authUserId:resolved.owner.authUserId,customerId:resolved.owner.customerId,email:resolved.owner.email,sessionId:resolved.sessionId,rpcAuthSessionId:p.rpcAuthSessionId,securityEpoch:resolved.securityEpoch,authorizingPasskeyId:p.authorizingPasskeyId,authorizingCredentialIdHash:p.authorizingCredentialIdHash,activeCredentialSetHash:p.activeCredentialSetHash,authorizationMethod:p.authorizationMethod,emailVerificationHash:p.emailVerificationHash,pendingPasskeyId:String(pending.id),rotationId,publicKeySha256,serverDeviceBindingKeyId:p.serverDeviceBindingKeyId,deviceBindingKeyId:device.keyId,pendingExpiresAtMs:actualExpiry,expiresAtMs:Math.min(actualExpiry,Date.now()+DIRAC_SECURITY_PASSKEY_RESET_TOKEN_TTL_MS_V363)});
    return{ok:true,method:'passkey',passkeyMode:'confirm_pending',reset_passkey:true,confirmation_required:true,database_saved:true,pending_created:true,registered_now:false,device_binding_required:true,device_binding_policy:DIRAC_PASSKEY_SERVER_DEVICE_BINDING_POLICY_V367,setupToken:issued.token,mfaSetupToken:issued.token,expires_in:Math.floor((issued.payload.expiresAtMs-Date.now())/1000),publicKey:{challenge:issued.payload.challenge,rpId:issued.payload.rpId,timeout:Math.min(60000,issued.payload.expiresAtMs-Date.now()),userVerification:'required',allowCredentials:[{type:'public-key',id:credentialId,transports:Array.isArray(pending.transports)?pending.transports:[]}]},credential_id_hint:diracSecurityPasskeyResetSha256V363(credentialId).slice(0,12),message:'Passkey pengganti tersimpan pending. Konfirmasikan Passkey baru sekali lagi.'};
...
async function diracSecurityPasskeyResetRestoreCurrentSessionV372(req, resolved, securityEpoch) {
  const readPath = '/rest/v1/security_customer_sessions?select=' + encodeURIComponent('id,customer_id,status,security_epoch,revoked_at,revoke_reason,expires_at')
    + '&customer_id=eq.' + encodeURIComponent(resolved.owner.customerId)
    + '&id=eq.' + encodeURIComponent(resolved.sessionId) + '&limit=2';
  const before = await supabaseFetch(readPath, { method: 'GET', auth: 'service' });
  const beforeRows = before && before.ok === true && Array.isArray(before.data) ? before.data : [];
  const row = beforeRows.length === 1 ? beforeRows[0] : null;
  ...
  if (!row || !safeEqual(String(row.id || ''), resolved.sessionId) || !safeEqual(String(row.customer_id || ''), resolved.owner.customerId)
      || Number(row.security_epoch || 0) !== Number(securityEpoch) || !Number.isFinite(expiresAtMs) || expiresAtMs <= Date.now()) {
    throw resetError('SECURITY_PASSKEY_RESET_SESSION_CONTINUITY_PRECONDITION_FAILED', 503);
  }
  const context = diracCentralCurrentContextV149();
  if (!context || context.req !== req || !context.active || !context.browserChecked || !context.banChecked) throw resetError('SECURITY_PASSKEY_RESET_SESSION_CONTINUITY_CONTEXT_INVALID', 503);
  if (String(row.status || '').toLowerCase() === 'active' && !row.revoked_at && !row.revoke_reason) {
    return Object.freeze({ sessionId: resolved.sessionId, securityEpoch: Number(securityEpoch), expiresAtMs });
  }
  if (String(row.status || '').toLowerCase() !== 'revoked' || !row.revoked_at || String(row.revoke_reason || '') !== 'passkey_rotation_v237') {
    throw resetError('SECURITY_PASSKEY_RESET_SESSION_CONTINUITY_PRECONDITION_FAILED', 503);
  }
  context.__diracPasskeyResetSessionContinuityV372 = Object.freeze({ active: true, sessionId: resolved.sessionId, securityEpoch: Number(securityEpoch) });
  try {
    const patchPath = '/rest/v1/security_customer_sessions?customer_id=eq.' + encodeURIComponent(resolved.owner.customerId)
      + '&id=eq.' + encodeURIComponent(resolved.sessionId)
      + '&status=eq.revoked&security_epoch=eq.' + encodeURIComponent(String(securityEpoch))
      + '&revoke_reason=eq.passkey_rotation_v237';
    const patched = await supabaseFetch(patchPath, { method: 'PATCH', auth: 'service', prefer: 'return=representation', body: { status: 'active', revoked_at: null, revoke_reason: null } });
    const rows = patched && patched.ok === true && Array.isArray(patched.data) ? patched.data : [];
    const restored = rows.length === 1 ? rows[0] : null;
    const restoredExpiry = Date.parse(String(restored && restored.expires_at || ''));
    if (!restored || !safeEqual(String(restored.id || ''), resolved.sessionId) || !safeEqual(String(restored.customer_id || ''), resolved.owner.customerId)
        || String(restored.status || '').toLowerCase() !== 'active' || restored.revoked_at || restored.revoke_reason
        || Number(restored.security_epoch || 0) !== Number(securityEpoch) || !Number.isFinite(restoredExpiry) || restoredExpiry <= Date.now()) {
      throw resetError('SECURITY_PASSKEY_RESET_SESSION_CONTINUITY_POSTCONDITION_FAILED', patched && patched.ok === true ? 409 : 503);
    }
    return Object.freeze({ sessionId: resolved.sessionId, securityEpoch: Number(securityEpoch), expiresAtMs: restoredExpiry });
  } finally {
    context.__diracPasskeyResetSessionContinuityV372 = null;
  }
}

function diracSecurityPasskeyResetContinuityCookieV382(resolved, securityEpoch, sessionTokenHash, rotationId) {
  const now = Math.floor(Date.now() / 1000);
  const signedExpiresAt = Number(resolved && resolved.signedExpiresAt || 0);
  const epoch = Number(securityEpoch || 0);
  const sid = String(resolved && resolved.sessionId || '').trim();
  const uid = String(resolved && resolved.owner && resolved.owner.authUserId || '').trim();
  const cid = String(resolved && resolved.owner && resolved.owner.customerId || '').trim();
  const rid = String(rotationId || '').trim();
  const sth = String(sessionTokenHash || '').trim();
  const exp = Math.min(now + 90, signedExpiresAt);
  if (!customerSecurityLooksLikeUuid(uid) || !customerSecurityLooksLikeUuid(cid) || !customerSecurityLooksLikeUuid(sid)
      || !customerSecurityLooksLikeUuid(rid) || !/^[a-f0-9]{64}$/.test(sth)
      || !Number.isSafeInteger(epoch) || epoch < 2 || epoch !== Number(resolved && resolved.securityEpoch || 0) + 1
      || !Number.isSafeInteger(signedExpiresAt) || signedExpiresAt <= now || !Number.isSafeInteger(exp) || exp <= now || exp > now + 90) {
    throw resetError('SECURITY_PASSKEY_RESET_CONTINUITY_PROOF_CONTEXT_INVALID', 503);
  }
  const payload = { typ: 'dirac-passkey-rotation-session-continuity-v382', uid, cid, sid, rid, sth, epoch, iat: now, exp, sexp: signedExpiresAt, nonce: crypto.randomBytes(24).toString('base64url') };
  const body = Buffer.from(JSON.stringify(payload)).toString('base64url');
  const key = diracCentralDeriveSecretV146('passkey-rotation-session-continuity-v382');
  let signature = '';
  try { signature = crypto.createHmac('sha256', key.toString('base64url')).update(body).digest('base64url'); }
  finally { key.fill(0); }
  const value = body + '.' + signature;
  if (!/^[A-Za-z0-9_-]{40,4096}\.[A-Za-z0-9_-]{43}$/.test(value)) throw resetError('SECURITY_PASSKEY_RESET_CONTINUITY_PROOF_SIGNING_FAILED', 503);
  return '__Host-dirac_pk_rotation_continuity_v382=' + encodeURIComponent(value) + '; Path=/; Max-Age=' + String(exp - now) + '; HttpOnly; Secure; SameSite=Strict; Priority=High';
}

async function diracSecurityPasskeyResetAppendContinuityCookiesV372(req, res, resolved, securityEpoch, rotationId) {
  const now = Math.floor(Date.now() / 1000);
  const signedIssuedAt = Number(resolved.signedIssuedAt || 0);
  const signedExpiresAt = Number(resolved.signedExpiresAt || 0);
  const originalMfaExpiresAt = Number(resolved.mfaExpiresAt || 0);
  const epoch = Number(securityEpoch || 0);
  if (!res || typeof res.setHeader !== 'function' || !Number.isSafeInteger(signedIssuedAt) || signedIssuedAt <= 0
      || !Number.isSafeInteger(signedExpiresAt) || signedExpiresAt <= now || signedExpiresAt - signedIssuedAt <= 0 || signedExpiresAt - signedIssuedAt > 8 * 60 * 60
      || !Number.isSafeInteger(originalMfaExpiresAt) || originalMfaExpiresAt <= now || originalMfaExpiresAt > signedExpiresAt
      || !Number.isSafeInteger(epoch) || epoch !== Number(resolved.securityEpoch) + 1) throw resetError('SECURITY_PASSKEY_RESET_SESSION_COOKIE_CONTEXT_INVALID', 503);
  const anchorExpiresAt = Math.min(originalMfaExpiresAt, signedExpiresAt);
  if (anchorExpiresAt <= now) throw resetError('SECURITY_PASSKEY_RESET_SESSION_COOKIE_EXPIRED', 503);
  const anchorId = crypto.randomBytes(32).toString('base64url');
  const signedPayload = {
    typ: 'dirac-domain-signed-session-v1', uid: resolved.owner.authUserId, email: resolved.owner.email, sid: resolved.sessionId,
    iat: signedIssuedAt, exp: signedExpiresAt, nonce: crypto.randomBytes(12).toString('base64url'), session_version: String(epoch),
    mfa_bid_v228: anchorId, mfa_iat_v228: now, mfa_exp_v228: anchorExpiresAt, mfa_epoch_v235: epoch
  };
  const signedBody = Buffer.from(JSON.stringify(signedPayload)).toString('base64url');
  const signedKey = diracCentralDeriveSecretV146('domain-signed-session');
  let signedSignature = '';
  try { signedSignature = crypto.createHmac('sha256', signedKey.toString('base64url')).update(signedBody).digest('base64url'); }
  finally { signedKey.fill(0); }
  const signedValue = signedBody + '.' + signedSignature;
  const binding = securityTrustCurrentDeviceBindingHashV366('signed_session_anchor_v228', JSON.stringify([DIRAC_SECURITY_TRUST_ANCHOR_V366, resolved.owner.authUserId, resolved.owner.email, anchorId, epoch]));
  const profileKey = diracCentralDeriveSecretV146('customer-mfa');
  let emailHash = '';
  try { emailHash = crypto.createHmac('sha256', profileKey.toString('base64url')).update('dirac-customer-mfa-profile-v1:' + resolved.owner.email).digest('hex'); }
  finally { profileKey.fill(0); }
  const mfaPayload = {
    type: DIRAC_SECURITY_TRUST_MFA_TYPE_V366, method: 'passkey', emailHash,
    authUserIdHash: securityTrustCurrentDeviceBindingHashV366('auth_user_id', resolved.owner.authUserId),
    customerIdHash: securityTrustCurrentDeviceBindingHashV366('customer_id', resolved.owner.customerId),
    sessionBindingVersion: 5, sessionHash: binding, securityEpoch: epoch, jti: crypto.randomBytes(24).toString('base64url'),
    activeAtMs: Date.now(), expiresAtMs: anchorExpiresAt * 1000,
    originHash: securityTrustCurrentDeviceBindingHashV366('origin', requestOrigin(req)),
    uaHash: securityTrustCurrentDeviceBindingHashV366('ua', String(requestUserAgent(req) || '').trim().slice(0, 512)), recoveryVerified: false
  };
  const mfaBody = Buffer.from(JSON.stringify(mfaPayload)).toString('base64url');
  const mfaValue = mfaBody + '.' + securityTrustCurrentDeviceMfaSignatureV366(mfaBody);
  if (!/^[A-Za-z0-9_-]+\.[A-Za-z0-9_-]{43}$/.test(signedValue) || !/^[A-Za-z0-9_-]+\.[A-Za-z0-9_-]{43}$/.test(mfaValue)
      || !/^[a-f0-9]{64}$/.test(binding) || !/^[a-f0-9]{64}$/.test(emailHash)) throw resetError('SECURITY_PASSKEY_RESET_SESSION_COOKIE_SIGNING_FAILED', 503);
  const signedCookieName = String(process.env.DOMAIN_SIGNED_SESSION_COOKIE || 'dirac_domain_signed_session').trim();
  const mfaCookieName = String(process.env.DIRAC_CUSTOMER_MFA_COOKIE || 'dirac_customer_mfa_session').trim();
  if (!/^[!#$%&'*+\-.^_`|~0-9A-Za-z]{1,128}$/.test(signedCookieName) || !/^[!#$%&'*+\-.^_`|~0-9A-Za-z]{1,128}$/.test(mfaCookieName)) throw resetError('SECURITY_PASSKEY_RESET_SESSION_COOKIE_NAME_INVALID', 503);
  const sessionTokenHash = diracSecurityPasskeyResetSha256V363(signedValue);
  const context = diracCentralCurrentContextV149();
  if (!/^[a-f0-9]{64}$/.test(sessionTokenHash) || !context || context.req !== req || !context.active || !context.browserChecked || !context.banChecked
      || !context.__diracPasswordResetVerifiedOwnerV333 || !safeEqual(String(context.__diracPasswordResetVerifiedOwnerV333.customerId || ''), resolved.owner.customerId)) {
    throw resetError('SECURITY_PASSKEY_RESET_SESSION_REBIND_CONTEXT_INVALID', 503);
  }
  const lastSeenAt = new Date().toISOString();
  context.__diracPasskeyResetSessionTokenRebindV381 = Object.freeze({ active: true, sessionId: resolved.sessionId, securityEpoch: epoch, sessionTokenHash });
  try {
    const patchPath = '/rest/v1/security_customer_sessions?customer_id=eq.' + encodeURIComponent(resolved.owner.customerId)
      + '&id=eq.' + encodeURIComponent(resolved.sessionId)
      + '&status=eq.active&security_epoch=eq.' + encodeURIComponent(String(epoch)) + '&revoked_at=is.null';
    const patched = await supabaseFetch(patchPath, { method: 'PATCH', auth: 'service', prefer: 'return=representation', body: { session_token_hash: sessionTokenHash, last_seen_at: lastSeenAt } });
    const rows = patched && patched.ok === true && Array.isArray(patched.data) ? patched.data : [];
    const row = rows.length === 1 ? rows[0] : null;
    const expiresAtMs = Date.parse(String(row && row.expires_at || ''));
    ...
    if (!row || !safeEqual(String(row.id || ''), resolved.sessionId) || !safeEqual(String(row.customer_id || ''), resolved.owner.customerId)
        ...
        || row.revoked_at || Number(row.security_epoch || 0) !== epoch || !Number.isFinite(expiresAtMs) || expiresAtMs <= Date.now()
        || !Number.isFinite(lastSeenMs) || Math.abs(Date.parse(lastSeenAt) - lastSeenMs) > 1000) {
      throw resetError('SECURITY_PASSKEY_RESET_SESSION_REBIND_POSTCONDITION_FAILED', patched && patched.ok === true ? 409 : 503);
    }
  } finally {
    context.__diracPasskeyResetSessionTokenRebindV381 = null;
  }
  const signedCookie = signedCookieName + '=' + encodeURIComponent(signedValue) + '; Path=/; Max-Age=' + String(signedExpiresAt - now) + '; HttpOnly; Secure; SameSite=Strict; Priority=High';
  const mfaCookie = mfaCookieName + '=' + encodeURIComponent(mfaValue) + '; Path=/; Max-Age=' + String(anchorExpiresAt - now) + '; HttpOnly; Secure; SameSite=Strict; Priority=High';
  const continuityCookie = diracSecurityPasskeyResetContinuityCookieV382(resolved, epoch, sessionTokenHash, rotationId);
  ...
  res.setHeader('Set-Cookie', existing.concat(signedCookie, mfaCookie, continuityCookie));
  return Object.freeze({ sessionId: resolved.sessionId, securityEpoch: epoch, signedExpiresAt, mfaExpiresAt: anchorExpiresAt, sessionTokenHash });
}

async function diracSecurityPasskeyResetConfirmV363(req,res,body,resolved,tokenData) {
  ...
  try{
    ...
    const finalBinding=diracSecurityPasskeyResetStoredServerDeviceBindingV367(newRow);if(!finalBinding.ok||!safeEqual(finalBinding.keyId,p.deviceBindingKeyId))throw resetError('SECURITY_PASSKEY_RESET_FINAL_BINDING_INVALID',503);
    const restoredSession=await diracSecurityPasskeyResetRestoreCurrentSessionV372(req,resolved,newEpoch);
    const continuity=await diracSecurityPasskeyResetAppendContinuityCookiesV372(req,res,resolved,newEpoch,p.rotationId);
    if(!restoredSession||!continuity||!safeEqual(String(restoredSession.sessionId||''),resolved.sessionId)||!safeEqual(String(continuity.sessionId||''),resolved.sessionId)||Number(restoredSession.securityEpoch)!==newEpoch||Number(continuity.securityEpoch)!==newEpoch)throw resetError('SECURITY_PASSKEY_RESET_SESSION_CONTINUITY_FAILED',503);
    const notice=await diracSecurityPasskeyResetSendChangedNoticeV372(req,resolved.owner,p.rotationId,newEpoch).catch(()=>Object.freeze({ok:false,provider:'',status:0,code:'SECURITY_PASSKEY_RESET_NOTICE_FAILED'}));
    return{ok:true,verified:true,method:'passkey',passkeyMode:'registration',reset_passkey:true,database_saved:true,registered_now:true,old_passkeys_deactivated:1,device_binding_verified:true,device_binding_policy:DIRAC_PASSKEY_SERVER_DEVICE_BINDING_POLICY_V367,security_epoch:newEpoch,credential_id_hint:diracSecurityPasskeyResetSha256V363(newRow.credential_id).slice(0,12),login_required:false,session_continued:true,notification_sent:Boolean(notice&&notice.ok===true),notification_provider:String(notice&&notice.provider||''),message:'Passkey berhasil diganti. Sesi perangkat ini tetap aktif dengan security epoch terbaru.'};
  }finally{if(ctx){ctx.__diracPasskeyResetAuthorizedV363=false;ctx.__diracPasskeyResetAuthorizationMethodV364='';ctx.__diracPasskeyResetEmailVerificationHashV364='';ctx.__diracPasskeyResetAuthorizingCredentialHashV363='';ctx.__diracPasskeyResetAuthSessionIdV363='';}}
}
...
async function diracSecurityPasskeyResetVerifyV363(req,res,body) {
  ...
  if(phase==='email'){
    ...
    return diracSecurityPasskeyResetEmailAuthorizeV364(req,body,resolved,tokenData);
  }
  ...
  if(phase==='authorize')return diracSecurityPasskeyResetAuthorizeOldV371(req,body,resolved,tokenData);if(phase==='register')return diracSecurityPasskeyResetRegisterV363(req,body,resolved,tokenData);return diracSecurityPasskeyResetConfirmV363(req,res,body,resolved,tokenData);
}

async function diracSecurityPasskeyResetEngineV363(req, res, body) {
  const action=String(body&&body.action||'').trim();
  if(action==='security_passkey_reset_start')return diracSecurityPasskeyResetStartV363(req,res,body);
  if(action==='security_passkey_reset_verify')return diracSecurityPasskeyResetVerifyV363(req,res,body);
...
function diracPasswordResetOpsV333(req) {
  return Object.freeze({
    ...
    deviceBindingContext: (email, credentialId) => diracPasswordResetDeviceBindingContextV374(req, email, credentialId),
...
function diracSecurityPasskeyResetServerDeviceAuthorityV367() {
  const authority=centralHandler&&centralHandler.__diracPasskeyServerDeviceBindingAuthorityV360;
  if(!authority||authority.version!=='dirac-passkey-server-device-binding-authority-v360'
      ||authority.bindingVersion!==DIRAC_PASSKEY_SERVER_DEVICE_BINDING_VERSION_V367
      ||authority.bindingAlgorithm!==DIRAC_PASSKEY_SERVER_DEVICE_BINDING_ALGORITHM_V367
      ||authority.bindingPolicy!==DIRAC_PASSKEY_SERVER_DEVICE_BINDING_POLICY_V367
      ||typeof authority.read!=='function'||typeof authority.ensure!=='function') throw resetError('SECURITY_PASSKEY_RESET_DEVICE_AUTHORITY_INVALID',503);
  return authority;
}
...
function securityResetCentralBanAuthorityV354() {
  ...
  if (!authority || authority.version !== 'dirac-central-ban-authority-v354' || typeof authority.check !== 'function' || typeof authority.ban !== 'function') {
...
async function securityResetCheckCentralBanV354(req) {
  ...
  const context = diracCentralCurrentContextV149();
  if (context && context.req === req) context.banChecked = true;
  return true;
}
...
async function handleStandalonePasskeyResetPostV363(req,res,parsed){
  ...
  req.query=Object.freeze({action:parsed.action});
  const result=await diracSecurityPasskeyResetEngineV363(req,res,body);
...
function parseExactRequest(req) {
  ...
  for (const [key] of params) {
    if (seen.has(key)) return { ok: false, code: 'SECURITY_ROUTE_QUERY_DUPLICATE' };
    seen.add(key);
  }
  if (params.toString() !== rawQuery) return { ok: false, code: 'SECURITY_ROUTE_QUERY_NON_CANONICAL' };
...
async function keamananDispatchV361(req, res) {
...
async function keamananHandler(req, res) {
  const context = { req, startedAt: Date.now(), banChecked: false, browserChecked: false, action: '', active: true };
  return SECURITY_RESET_REQUEST_CONTEXT_V361.run(context, async () => {
    try { return await keamananDispatchV361(req, res); }
...
if (process.env.NODE_ENV === 'test') Object.defineProperty(keamananHandler, '__test', { value: Object.freeze({
  parseExactRequest, securityResetReadJsonV334, securityResetVerifyCsrfV334, securityResetIssueCsrfV334,
  securityResetIssuePageNonceV334, securityResetVerifyPageNonceV334, securityResetApplyHeadersV334,
  diracPasswordResetSealX25519PrivateV333, diracPasswordResetUnsealX25519PrivateV333,
