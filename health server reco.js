...
const { parseStrictJson: diracParseStrictJsonV356, validUnicode: diracValidUnicodeV356 } = (() => {
'use strict';
...
const { trustedClientIp: diracTrustedClientIpV356, normalizeIp: diracNormalizeIpV356 } = (() => {
'use strict';
...
function hpkeExtract(hash, salt, ikm) {
  if (!['sha256', 'sha512'].includes(hash)) throw fail('HPKE_HASH_INVALID');
...
function hpkeExpand(hash, prk, info, length) {
  ...
  for (let index = 1; total < length; index += 1) {
    ...
    // Keep completed blocks intact until the complete output is copied.
    previous = next;
    blocks.push(next);
    total += next.length;
  }
...
function validateEnvelope(body, expectedHpkeKeyId, expectedMlkemKeyId) {
  ...
  assertExactObjectKeys(source, ['action', 'version', 'request_id', 'hpke_suite', 'hpke_key_id', 'mlkem_key_id', 'sent_at_ms', 'expires_at_ms', 'enc', 'aead_nonce', 'ciphertext', 'mlkem_ciphertext'], 'HYBRID_ENVELOPE_FIELDS_INVALID');
  if (typeof source.sent_at_ms !== 'number' || typeof source.expires_at_ms !== 'number') throw fail('HYBRID_ENVELOPE_TIME_INVALID');
  const now = Date.now();
  ...
  decodeB64u(source.mlkem_ciphertext, 1568, 4096).fill(0);
...
function parseHybridPlaintext(plaintext, requestId) {
  ...
  try {
    ...
    parsed = diracParseStrictJsonV356(text, { maxBytes: 256 * 1024 });
...
function claimHash(body, bundle) {
  const requestId = String(body && body.request_id || '');
  if (!requestId || !body || !bundle || !bundle.transport) throw fail('RECOVERY_CLAIM_BINDING_INVALID');
  const material = Buffer.from(jcs({
    action: String(body.action || ''),
    version: String(body.version || ''),
    request_id: requestId,
    hpke_suite: String(body.hpke_suite || ''),
    hpke_key_id: String(body.hpke_key_id || ''),
    mlkem_key_id: String(body.mlkem_key_id || ''),
    enc: String(body.enc || ''),
    aead_nonce: String(body.aead_nonce || ''),
    ciphertext: String(body.ciphertext || ''),
    sent_at_ms: Number(body.sent_at_ms || 0),
    expires_at_ms: Number(body.expires_at_ms || 0),
    idempotency_key: String(body.idempotency_key || ''),
    mlkem_ciphertext_sha512: String(bundle.transport.mlkem_ciphertext_sha512 || '')
  }), 'utf8');
  try {
    return sha512B64u(material);
  } finally {
    material.fill(0);
  }
}

function encryptResponse(responseKey, transcriptHash, options) {
  const source = options && typeof options === 'object' && !Array.isArray(options) ? options : {};
  const requestId = String(source.requestId || '');
  const customerId = String(source.customerId || '');
  const authUserId = String(source.authUserId || '');
  const recoveryCode = String(source.recoveryCode || '');
  const recoverySessionToken = String(source.recoverySessionToken || '');
  const recoverySessionExpiresAt = String(source.recoverySessionExpiresAt || '');
  if (!requestId || !customerId || !authUserId
      || !/^[A-Za-z0-9_-]{1200}$/.test(recoveryCode)
      || !/^[A-Za-z0-9_-]{32,160}$/.test(recoverySessionToken)
      || !Number.isFinite(Date.parse(recoverySessionExpiresAt))) {
    throw fail('SEALED_RECOVERY_INPUT_INVALID');
  }
  const ownerBindingPayload = {
    owner_auth_user_id: authUserId,
    owner_customer_id: customerId,
    purpose: PURPOSE,
    request_id: requestId
  };
  const ownerBinding = crypto.createHmac('sha512', responseKey)
    .update('dirac/recovery/v281/owner-binding\n', 'utf8')
    .update(jcs(ownerBindingPayload), 'utf8')
    .digest('base64url');
  const aadObject = {
    version: RESPONSE_VERSION,
    purpose: PURPOSE,
    request_id: requestId,
    owner_binding_sha512: ownerBinding
  };
  const aad = Buffer.from(jcs(aadObject), 'utf8');
  const plaintext = Buffer.from(jcs({
    ...
    recovery_session_expires_at: recoverySessionExpiresAt
  }), 'utf8');
  try {
    ...
    try {
      if (checkedCiphertext.length < 17 || !/^[A-Za-z0-9_-]{86}$/.test(sealed.owner_binding_sha512)) {
        throw fail('SEALED_RECOVERY_CIPHERTEXT_LENGTH_INVALID');
      }
      return sealed;
    } finally {
      checkedNonce.fill(0);
...
const DEFAULT_ALLOWED_ORIGINS = [
  'https://shop.' + diracBaseDomainV250(),
...
function makeCookie(name, value, options = {}) {
  ...
  // (misal configured deployment domain -> provider deployment domain). Untuk dev HTTP lokal, set env DOMAIN_COOKIE_SAMESITE=Lax.
...
function extractUserForSignedDomainSession(session) {
  // Only the user returned by the authoritative authentication API is eligible.
  // Unverified token contents never become a signed identity.
  const user = session && session.user;
  if (!user || typeof user !== 'object' || Array.isArray(user)) return null;
  const id = String(user.id || '').trim();
  const email = normalizeAuthEmail(user.email || '');
  if (!customerSecurityLooksLikeUuid(id) || !email) return null;
  return { id, email };
}
...
function customerSecurityLostPasskeyOperationBudgetMsV281() {
  const raw = Number(process.env.DIRAC_LOST_PASSKEY_OPERATION_BUDGET_MS || 180000);
  if (!Number.isFinite(raw)) return 180000;
  return Math.max(60000, Math.min(210000, Math.floor(raw)));
}

/* source 7300-7302 */
function customerSecurityLostPasskeyQueueMaxWaitMsV164() {
  const budgetCeilingSeconds = Math.max(15, Math.floor((customerSecurityLostPasskeyOperationBudgetMsV281() - 30000) / 1000));
  return customerSecurityLostPasskeyQueueIntV164(
    'DIRAC_LOST_PASSKEY_QUEUE_MAX_WAIT_SECONDS',
    Math.min(120, budgetCeilingSeconds),
    15,
    budgetCeilingSeconds
  ) * 1000;
}
...
function customerSecurityLostPasskeyQueueMaxWaitForTaskMsV191(queueTask) {
  ...
  return task === DIRAC_RECOVERY_HPKE_VERIFY_ACTION_V159
    || task === DIRAC_RECOVERY_WORKER_TASK_VERIFY
    ? 5000
...
function customerSecurityRecoveryWorkerOpenV190(envelope, caller, timestampText) {
  ...
  try {
    ...
    const parsed = diracParseStrictJsonV356(text, { maxBytes: 256 * 1024 });
...
function customerSecurityLostPasskeyArgon2ProfilesV191() {
  const configuredMemory = Math.min(customerSecurityLostPasskeyArgon2EnvIntegerV191(
    'DIRAC_LOST_PASSKEY_ARGON2_MEMORY_KIB', DIRAC_RECOVERY_ARGON2_MEMORY_KIB_V266, DIRAC_RECOVERY_ARGON2_MEMORY_KIB_V266, 5242880
  ), DIRAC_RECOVERY_ARGON2_MEMORY_KIB_V266);
  ...
  const hpkeMinimumMemory = Math.min(customerSecurityLostPasskeyArgon2EnvIntegerV191(
    'DIRAC_RECOVERY_HPKE_ARGON2_MEMORY_KIB', configuredMemory, DIRAC_RECOVERY_ARGON2_MEMORY_KIB_V266, 5242880
  ), DIRAC_RECOVERY_ARGON2_MEMORY_KIB_V266);
  ...
  const linkOpen = Object.freeze({
    memoryCost: Math.min(customerSecurityLostPasskeyArgon2EnvIntegerV191(
      'DIRAC_LOST_PASSKEY_LINK_OPEN_ARGON2_MEMORY_KIB', main.memoryCost, DIRAC_RECOVERY_ARGON2_MEMORY_KIB_V266, 5242880
    ), DIRAC_RECOVERY_ARGON2_MEMORY_KIB_V266),
...
function customerSecurityLostPasskeySecretsForMetadataV281(metadata, currentSecrets) {
  const current = currentSecrets && currentSecrets.ok === true
    ? currentSecrets
    : customerSecurityLostPasskeyRequireVaultSecretsV157();
  if (!current.ok) return current;
  const declaredVersion = String(metadata && metadata.root_secret_version || '').trim();
  if (!/^[A-Za-z0-9_.-]{1,80}$/.test(declaredVersion)) {
    return { ok: false, code: 'LOST_PASSKEY_ROOT_SECRET_VERSION_MISSING', message: 'Versi recovery secret tidak valid.' };
  }
  if (declaredVersion === current.rootSecretVersion) return current;

  const previousSecret = String(process.env.DIRAC_SECURITY_ROOT_SECRET_PREVIOUS || '').normalize('NFC');
  const previousVersion = String(process.env.DIRAC_SECURITY_ROOT_SECRET_PREVIOUS_VERSION || '').trim();
  const previousConfigured = previousSecret.length > 0 || previousVersion.length > 0;
  if (!previousConfigured
      || Buffer.byteLength(previousSecret, 'utf8') < LOST_PASSKEY_ROOT_SECRET_MIN_BYTES_V157
      || !/^[A-Za-z0-9_.-]{1,80}$/.test(previousVersion)
      || previousVersion === current.rootSecretVersion
      || declaredVersion !== previousVersion
      || crypto.createHash('sha512').update(previousSecret, 'utf8').digest('base64url')
        === crypto.createHash('sha512').update(current.rootSecret, 'utf8').digest('base64url')) {
    return { ok: false, code: 'LOST_PASSKEY_ROOT_SECRET_VERSION_UNAVAILABLE', message: 'Versi recovery secret tidak tersedia.' };
  }

  const explicitPepper = String(process.env.DIRAC_LOST_PASSKEY_DB_PEPPER || '').normalize('NFC');
  const pepper = Buffer.byteLength(explicitPepper, 'utf8') >= LOST_PASSKEY_DB_PEPPER_MIN_BYTES_V157
    ? explicitPepper
    : crypto.createHmac('sha512', previousSecret)
      .update('dirac-lost-passkey-db-pepper-v157')
      .digest('base64url');
  if (Buffer.byteLength(pepper, 'utf8') < LOST_PASSKEY_DB_PEPPER_MIN_BYTES_V157) {
    return { ok: false, code: 'LOST_PASSKEY_DB_PEPPER_INVALID', message: 'Konfigurasi recovery vault belum valid.' };
  }
  return {
    ok: true,
    rootSecret: previousSecret,
    pepper,
    rootSecretVersion: previousVersion
  };
}
...
/* source 8768-8777 | ARGON2 PHC PARAMETER FIX v280 COMPAT */
function customerSecurityLostPasskeyArgon2EncodedParamsV171(encodedHash) {
  const hash = String(encodedHash || '');
  // Producers can emit the same PHC parameter set in a different order.
  // Bind the exact, unique m/t/p set instead of binding to serialization order.
  if (!hash || hash.length > 8192) return null;
  const match = /^\$argon2id\$v=19\$([mtp]=\d{1,8}(?:,[mtp]=\d{1,8}){2})\$([A-Za-z0-9+/]+)\$([A-Za-z0-9+/]+)$/.exec(hash);
  if (!match) return null;
  const values = Object.create(null);
  for (const field of match[1].split(',')) {
    const pair = /^([mtp])=([1-9]\d{0,7})$/.exec(field);
    if (!pair || Object.prototype.hasOwnProperty.call(values, pair[1])) return null;
    values[pair[1]] = Number(pair[2]);
  }
  if (Object.keys(values).sort().join('') !== 'mpt') return null;
  const memoryCost = values.m;
  const timeCost = values.t;
  const parallelism = values.p;
  let salt = null;
  let digest = null;
  try {
    if (match[2].length % 4 === 1 || match[3].length % 4 === 1) return null;
    salt = Buffer.from(match[2], 'base64');
    digest = Buffer.from(match[3], 'base64');
    if (salt.toString('base64').replace(/=+$/, '') !== match[2]
        || digest.toString('base64').replace(/=+$/, '') !== match[3]
        || salt.length !== LOST_PASSKEY_RECOVERY_SALT_BYTES_V157
        || digest.length !== 64
    ...
    return { memoryCost, timeCost, parallelism, hashLength: 64 };
  } finally {
...
/* source 9643-9653 */
async function customerSecurityVerifyAccountPasswordForPdfV156(email, accountPassword, expectedAuthUserId) {
  const normalizedEmail = normalizeAuthEmail(email);
  const password = String(accountPassword || '');
  if (!isValidAuthEmail(normalizedEmail) || !password || !customerSecurityLooksLikeUuid(expectedAuthUserId)) return { ok: false, status: 400 };
  ...
  return await new Promise((resolve, reject) => {
    ...
    const onData = (chunk) => {
      ...
      if (Buffer.byteLength(buffer, 'utf8') > 64 * 1024) {
        cleanup(reject, new Error('SMTP_RESPONSE_TOO_LARGE'));
        return;
      }
...
function diracRoleOriginV250(role) {
  ...
  throw new Error('DIRAC_TARGET_ROLE_INVALID');
}
...
async function customerSecuritySendLostPasskeyEmailCodeV342(to, context = {}) {
  ...
  if (!isValidAuthEmail(email) || !requestId || !emailSecret
    ...
    return { ok: false, status: 400, code: 'RECOVERY_EMAIL100_INPUT_INVALID', message: 'Material kode email recovery tidak valid.' };
  }
  const config = customerSecurityRecoverySmtpConfig();
  if (!config || String(config.host || '').trim().toLowerCase() !== 'smtp.gmail.com'
      || Number(config.port) !== 465 || config.secure !== true) {
    return { ok: false, status: 503, code: 'RECOVERY_SMTP_CONFIGURATION_MISSING', message: 'SMTP Gmail recovery belum dikonfigurasi.' };
  }
  const fromEmail = normalizeAuthEmail(config.user);
  if (!isValidAuthEmail(fromEmail)) return { ok: false, status: 503, code: 'RECOVERY_SMTP_FROM_INVALID', message: 'Akun SMTP recovery tidak valid.' };
...
async function customerSecuritySendLostPasskeyRecoveryLinkEmailV157(to, context = {}) {
  ...
  if (process.env.RESEND_API_KEY) {
    try {
      const response = await diracRecoveryEgressFetchV356('https://api.resend.com/emails', {
        method: 'POST',
  ...
  if (process.env.BREVO_API_KEY) {
    try {
      ...
      const response = await diracRecoveryEgressFetchV356('https://api.brevo.com/v3/smtp/email', {
        method: 'POST',
...
async function customerSecurityGenerateEmail100RecoveryV342(req, res, action, access, owner, activePasskeys, bindings, vaultSecrets, requestId, nowIso, expiresAt, queueTicket) {
  ...
  const rowSalt = crypto.randomBytes(32);
  const bindingsCanonical = customerSecurityLostPasskeyCanonical(bindings);
  let emailSecretHash = '';
  let bindingHashCommitment = '';
  const verifierKeyV347 = crypto.createHmac('sha512', Buffer.from(vaultSecrets.rootSecret, 'utf8'))
    .update('dirac-lost-passkey-email100-verifier-key-v347\n', 'utf8')
    .update(String(vaultSecrets.pepper || ''), 'utf8')
    .digest();
  try {
    emailSecretHash = crypto.createHmac('sha512', verifierKeyV347)
      ...
      .digest('hex');
    bindingHashCommitment = crypto.createHmac('sha512', verifierKeyV347)
      .update('binding\n', 'utf8')
      .update(customerSecurityLostPasskeyCanonical({
        request_id: requestId,
        customer_id: owner.customerId,
  ...
  if (!/^[a-f0-9]{128}$/.test(emailSecretHash) || !/^[a-f0-9]{128}$/.test(bindingHashCommitment)) {
    emailSecretSalt.fill(0);
    bindingSalt.fill(0);
    rowSalt.fill(0);
    return res.status(503).json({
      ok: false,
      code: 'RECOVERY_EMAIL100_VERIFIER_FAILED',
      message: 'Verifier kriptografis recovery belum dapat disiapkan.'
    });
  }
  const stateAad = Buffer.from(customerSecurityLostPasskeyCanonical({
    version: 'lost-passkey-password-email100-passkey-v342',
    request_id: requestId,
    customer_id: owner.customerId,
    auth_user_id: owner.authUserId,
    created_at: nowIso,
  ...
  const stateKeyMaterial = crypto.createHmac('sha512', Buffer.from(vaultSecrets.rootSecret, 'utf8'))
    .update('dirac-password-email100-passkey-state-v342\n', 'utf8')
    .update(requestId, 'utf8')
    .digest();
  const stateKey = Buffer.from(stateKeyMaterial.subarray(0, 32));
  const stateNonce = crypto.randomBytes(12);
  let stateCiphertext = null;
  let stateTag = null;
  try {
    const cipher = crypto.createCipheriv('aes-256-gcm', stateKey, stateNonce, { authTagLength: 16 });
    cipher.setAAD(stateAad, { plaintextLength: statePlaintext.length });
    stateCiphertext = Buffer.concat([cipher.update(statePlaintext), cipher.final()]);
    stateTag = cipher.getAuthTag();
  } finally {
    stateKeyMaterial.fill(0);
    stateKey.fill(0);
  ...
  const metadata = {
    ...
    recovery_mode: 'password_plus_email100_then_passkey',
    ...
    password_factor_required_on_generate: true,
    password_factor_required_on_verify: true,
  ...
  const stateCipherB64 = customerSecurityLostPasskeyB64(stateCiphertext);
  const stateNonceB64 = customerSecurityLostPasskeyB64(stateNonce);
  const stateTagB64 = customerSecurityLostPasskeyB64(stateTag);
  const stateSignature = customerSecurityLostPasskeyHmacHexV157(
    vaultSecrets.rootSecret,
    'email100_state_signature_v342',
    customerSecurityLostPasskeyCanonical({
      request_id: requestId,
      aad_sha256: customerSecurityLostPasskeySha256B64(stateAad),
      ciphertext_sha256: customerSecurityLostPasskeySha256B64(stateCiphertext),
      tag: stateTagB64
    })
  );
  ...
  const insertBody = [{
    ...
    recovery_code_hash: emailSecretHash,
    encrypted_file_key_text: stateCipherB64,
    file_key_wrap_nonce: stateNonceB64,
    file_key_wrap_tag: stateTagB64,
    salt: customerSecurityLostPasskeyB64(rowSalt),
    owner_key_salt: customerSecurityLostPasskeyB64(emailSecretSalt),
    file_sha256: customerSecurityLostPasskeySha256B64(stateCiphertext),
    aad_hash: customerSecurityLostPasskeySha256B64(stateAad),
    server_signature: stateSignature,
  ...
  try {
    const created = await supabaseFetch('/rest/v1/' + LOST_PASSKEY_RECOVERY_REQUEST_TABLE, {
      method: 'POST', auth: 'service', prefer: 'return=minimal', body: insertBody
    });
    if (!created.ok) {
      return res.status(created.status || 500).json({ ok: false, code: 'RECOVERY_REQUEST_INSERT_FAILED', message: 'Gagal menyimpan recovery request.' });
    }

    const deliveryPrepared = await supabaseFetch('/rest/v1/' + LOST_PASSKEY_RECOVERY_REQUEST_TABLE
    ...
    if (!deliveryPrepared.ok) {
      await supabaseFetch('/rest/v1/' + LOST_PASSKEY_RECOVERY_REQUEST_TABLE + '?request_id=eq.' + encodeURIComponent(requestId), {
        method: 'PATCH', auth: 'service',
        body: { status: 'revoked', sent_at: null, revoked_at: diracNowIso(), metadata: { ...metadata, delivery_precommit_failed: true, delivery_precommit_failed_at: diracNowIso() } }
      }).catch(() => null);
      return res.status(deliveryPrepared.status || 500).json({ ok: false, code: 'RECOVERY_EMAIL100_PRECOMMIT_FAILED', message: 'Kode email tidak dikirim karena status recovery belum dapat dikomit.' });
    }

    const sent = await customerSecuritySendLostPasskeyEmailCodeV342(owner.email, {
      requestId,
      expiresAt,
    ...
    if (!sent.ok) {
      await supabaseFetch('/rest/v1/' + LOST_PASSKEY_RECOVERY_REQUEST_TABLE + '?request_id=eq.' + encodeURIComponent(requestId), {
        method: 'PATCH', auth: 'service',
        body: { status: 'revoked', sent_at: null, revoked_at: diracNowIso(), metadata: { ...metadata, delivery_failed: true, delivery_failed_at: diracNowIso() } }
      }).catch(() => null);
      return res.status(sent.status || 503).json({ ok: false, code: sent.code || 'RECOVERY_EMAIL100_SEND_FAILED', message: sent.message || 'Kode email belum dapat dikirim.' });
    }
    ...
    stateAad.fill(0);
    stateNonce.fill(0);
    if (stateCiphertext) stateCiphertext.fill(0);
    if (stateTag) stateTag.fill(0);
  }
}
...
async function customerSecurityGenerateRecoveryCodes(req, res, action, override = null) {
  ...
  if (override && override.emailCodeOnlyV342 === true) {
    return customerSecurityGenerateEmail100RecoveryV342(
      req, res, action, access, owner, activePasskeys, bindings, vaultSecrets,
      requestId, nowIso, expiresAt, override.argonQueueTicket
    );
  }
...
async function diracV107DirectFetch(method, suffix, body) {
  ...
  if (!table) return { ok: false, status: 503, data: null };
  return supabaseFetch('/rest/v1/' + encodeURIComponent(table) + String(suffix || ''), {
    method: String(method || 'GET').toUpperCase(), auth: 'service',
    prefer: String(method || '').toUpperCase() === 'POST' ? 'resolution=merge-duplicates' : '', body
  });
}
...
function diracCsrfRequestBinding(req) {
  ...
  const tokenMaterial = [];
  if (signedUser) {
    tokenMaterial.push('signed-session', ...readCookieTokenCandidates(cookies, DOMAIN_SIGNED_SESSION_COOKIE));
  } else {
    try {
      tokenMaterial.push(
        ...readCookieTokenCandidates(cookies, ACCESS_COOKIE),
...
async function diracBolaIdorV128DirectSupabaseServiceGet(path) {
  return supabaseFetch(String(path || ''), { method: 'GET', auth: 'service' });
}
...
function diracV137CsrfForceVerify(req, action) {
  ...
  if (!Number.isSafeInteger(payload.exp) || !Number.isSafeInteger(payload.iat)
      || !Number.isFinite(skew) || payload.exp <= payload.iat
      || payload.exp - payload.iat > DIRAC_CSRF_MAX_AGE_SECONDS
      || payload.exp + skew < now) return { ok: false, status: 403, code: 'CSRF_TOKEN_EXPIRED' };
  if (payload.iat - skew > now) return { ok: false, status: 403, code: 'CSRF_TOKEN_IAT_INVALID' };
  try {
    const binding = diracCsrfRequestBinding(req);
    if (!binding || typeof payload.sid !== 'string' || !payload.sid || !binding.sid
        || !safeEqual(payload.sid, binding.sid)) return { ok: false, status: 403, code: 'CSRF_SESSION_BINDING_MISMATCH' };
    if (typeof payload.oh !== 'string' || !payload.oh || !binding.oh
        || !safeEqual(payload.oh, binding.oh)) return { ok: false, status: 403, code: 'CSRF_ORIGIN_BINDING_MISMATCH' };
  } catch (_) { return { ok: false, status: 403, code: 'CSRF_BINDING_UNAVAILABLE' }; }
...
/* source 28567-28830 */
async function diracRecoveryCompareAndSetRequestV356(row, owner, next) {
  const attemptCount = row && row.attempt_count == null ? 0 : row && row.attempt_count;
  const nextStatus = next && next.status;
  const allowedFields = nextStatus === 'verified' ? ['status', 'metadata'] : ['status', 'used_at', 'revoked_at', 'metadata'];
  if (!next || !next.metadata || typeof next.metadata !== 'object' || Array.isArray(next.metadata)
      || Object.keys(next).some((key) => !allowedFields.includes(key))
      || (nextStatus === 'verified' && !/^[a-f0-9]{64}$/.test(String(next.metadata.verification_session_hash || '')))) {
    return { ok: false, status: 409, data: null };
  }
  if (!row || !owner || !['pending', 'verified'].includes(row.status)
      || !['verified', 'used'].includes(nextStatus)
      || (nextStatus === 'used' && row.status !== 'verified')
      || row.used_at || row.revoked_at || row.locked_at
      || !Number.isSafeInteger(attemptCount) || attemptCount < 0
      || attemptCount > LOST_PASSKEY_RECOVERY_ATTEMPT_LIMIT
      || (nextStatus === 'verified' && attemptCount >= LOST_PASSKEY_RECOVERY_ATTEMPT_LIMIT)
      || row.customer_id !== owner.customerId || row.auth_user_id !== owner.authUserId
      || !Number.isFinite(Date.parse(row.expires_at)) || Date.parse(row.expires_at) <= Date.now()) {
    return { ok: false, status: 409, data: null };
  }
  const nextAttempts = nextStatus === 'verified' ? attemptCount + 1 : attemptCount;
  const attemptFilter = row.attempt_count == null ? 'is.null' : 'eq.' + attemptCount;
  const path = '/rest/v1/' + LOST_PASSKEY_RECOVERY_REQUEST_TABLE
    + '?select=' + encodeURIComponent('request_id,customer_id,auth_user_id,status,attempt_count,used_at,revoked_at,locked_at,metadata')
    + '&request_id=eq.' + encodeURIComponent(row.request_id)
    + '&customer_id=eq.' + encodeURIComponent(owner.customerId)
    + '&auth_user_id=eq.' + encodeURIComponent(owner.authUserId)
    + '&status=eq.' + row.status + '&attempt_count=' + attemptFilter
    + '&expires_at=gt.' + encodeURIComponent(new Date().toISOString())
    + '&used_at=is.null&revoked_at=is.null&locked_at=is.null';
  const result = await supabaseFetch(path, {
    method: 'PATCH', auth: 'service', prefer: 'return=representation',
    body: { ...next, attempt_count: nextAttempts }
  });
  const changed = result && result.ok === true && Array.isArray(result.data) && result.data.length === 1 ? result.data[0] : null;
  if (!changed || changed.request_id !== row.request_id || changed.customer_id !== owner.customerId
      || changed.auth_user_id !== owner.authUserId || changed.status !== nextStatus
      || changed.attempt_count !== nextAttempts || changed.locked_at
      || (nextStatus === 'verified' && (changed.used_at || changed.revoked_at
        || !changed.metadata || !safeEqual(String(changed.metadata.verification_session_hash || ''), String(next.metadata && next.metadata.verification_session_hash || ''))))
      || (nextStatus === 'used' && (Date.parse(changed.used_at) !== Date.parse(next.used_at) || Date.parse(changed.revoked_at) !== Date.parse(next.revoked_at)))) {
    return { ok: false, status: result && result.ok === false ? result.status || 503 : 409, data: null };
  }
...
async function customerSecurityVerifyRecoveryCodeLocalWorker(req, res, action, override = {}) {
  ...
  let vaultSecrets = customerSecurityLostPasskeyRequireVaultSecretsV157();
  if (!vaultSecrets.ok) {
  ...
  const attempts = row.attempt_count == null ? 0 : row.attempt_count;
  if (!Number.isSafeInteger(attempts) || attempts < 0 || attempts >= LOST_PASSKEY_RECOVERY_ATTEMPT_LIMIT
    || row.used_at
    || row.revoked_at
    || row.locked_at
  ...
  if (verifierModeV347) {
    ...
    if (emailCodeOnlyV342
        && (String(metadata.recovery_mode || '') !== 'password_plus_email100_then_passkey'
          || String(metadata.recovery_code_hash_label || '') !== 'email_secret'
          || String(metadata.email_secret_hash || '') !== String(row.recovery_code_hash || '')
          || Number(metadata.secret_email_length || 0) !== LOST_PASSKEY_SECRET_100_CHAR_LENGTH_V157
          || metadata.password_factor_required_on_verify !== true)) {
      await customerSecurityRegisterFailedVerification(req, action, 'recovery_email100_metadata_invalid', access.customerId).catch(() => null);
      return customerSecurityLostPasskeyGenericWorkerErrorV157(res, 403, 'recovery_email100_metadata_invalid', { request_id: requestId, customer_id: owner.customerId, auth_user_id: owner.authUserId, email: owner.email, worker_action: DIRAC_RECOVERY_WORKER_TASK_VERIFY }, { owner, bindings, requestId, code, row, metadata, bindingCommitmentOk: false, workerAction: DIRAC_RECOVERY_WORKER_TASK_VERIFY });
    }

    let legacyQueueTicketV350 = override && override.argonQueueTicket || null;
    let ownsLegacyQueueTicketV350 = false;
    if (!legacyQueueTicketV350) {
      legacyQueueTicketV350 = await customerSecurityLostPasskeyQueueAcquireV164(req, {
        nonce: requestId,
        caller_id: 'server2_recovery_verify_v350',
        queue_task: DIRAC_RECOVERY_WORKER_TASK_VERIFY
      });
      ownsLegacyQueueTicketV350 = Boolean(legacyQueueTicketV350 && legacyQueueTicketV350.ok);
    }
    if (!legacyQueueTicketV350 || !legacyQueueTicketV350.ok) {
      return res.status(legacyQueueTicketV350 && legacyQueueTicketV350.status || 503).json({
        ok: false,
        code: legacyQueueTicketV350 && legacyQueueTicketV350.code || 'RECOVERY_ARGON2_BUSY',
        message: 'Verifikasi recovery sedang diproses. Silakan coba kembali.'
      });
    }
    try {
      if (!customerSecurityLostPasskeyQueueLeaseHealthyV188(legacyQueueTicketV350)) {
      ...
      expectedBinding = await customerSecurityLostPasskeyArgon2VerifyHashV157(
        'binding', customerSecurityLostPasskeyCanonical(authoritativeBindings), metadata.binding_hash_commitment, vaultSecrets.pepper, vaultSecrets.rootSecret
      );
      codeOk = expectedBinding ? await customerSecurityLostPasskeyArgon2VerifyHashV157(
        emailCodeOnlyV342 ? 'email_secret' : 'recovery_code',
        code,
        emailCodeOnlyV342 ? String(metadata.email_secret_hash || '') : row.recovery_code_hash,
        vaultSecrets.pepper,
        vaultSecrets.rootSecret
      ) : false;
    } finally {
      if (ownsLegacyQueueTicketV350) {
        try { await legacyQueueTicketV350.release(); } catch (_) {}
      }
    }
  }
  ...
  if (!codeOk) {
    const failure = await diracRecoveryRegisterFailureV282(requestId, emailCodeOnlyV342 ? 'email_code_100' : 'recovery_code', action).catch(() => null);
    if (!failure || !['recorded', 'locked'].includes(failure.outcome)) {
  ...
  const verifiedMetadata = {
    ...
    verification_session_hash: recoverySessionHash,
  ...
  const patched = await diracRecoveryCompareAndSetRequestV356(row, owner, { status: 'verified', metadata: verifiedMetadata });
  ...
  if (!patched.ok) {
    ...
    const failedTrace = customerSecurityLostPasskeyWorkerVerifyTraceV174('verify_request_patch_failed', 'recovery_request_verify_patch_failed', {
      ...
      debugHint: 'Recovery request claim failed before session creation.'
    });
  ...
  if (verifiedRetry) {
    ...
    if (!previousSessionRevoked.ok || !Array.isArray(previousSessionRevoked.data)
        || previousSessionRevoked.data.some((item) => !item || item.status !== 'revoked' || item.used_at || Date.parse(item.revoked_at) !== Date.parse(now))) {
  ...
  if (!sessionCreated.ok || !Array.isArray(sessionCreated.data) || sessionCreated.data.length !== 1
      || sessionCreated.data[0].status !== 'verified' || Date.parse(sessionCreated.data[0].expires_at) !== Date.parse(sessionExpiresAt)) {
  ...
  const confirmed = await supabaseFetch('/rest/v1/' + LOST_PASSKEY_RECOVERY_REQUEST_TABLE
    + '?select=' + encodeURIComponent('request_id,status,used_at,revoked_at,locked_at,expires_at,metadata')
    + '&request_id=eq.' + encodeURIComponent(requestId) + '&limit=2', { method: 'GET', auth: 'service' });
  const confirmedRow = confirmed && confirmed.ok === true && Array.isArray(confirmed.data) && confirmed.data.length === 1 ? confirmed.data[0] : null;
  if (!confirmedRow || confirmedRow.request_id !== requestId || confirmedRow.status !== 'verified'
      || confirmedRow.used_at || confirmedRow.revoked_at || confirmedRow.locked_at
      || !Number.isFinite(Date.parse(confirmedRow.expires_at)) || Date.parse(confirmedRow.expires_at) <= Date.now()
      || !confirmedRow.metadata || !safeEqual(String(confirmedRow.metadata.verification_session_hash || ''), recoverySessionHash)) {
    return res.status(409).json({ ok: false, code: 'RECOVERY_VERIFICATION_STATE_CHANGED', message: 'Recovery request berubah. Ulangi recovery.' });
  }
  ...
  const responseBody = {
    ...
    valid: true,
    active: true,
    method: emailCodeOnlyV342 ? 'password_email_100' : 'recovery_code',
    purpose: LOST_PASSKEY_RECOVERY_PURPOSE,
    request_id: requestId,
    ...
    recovery_code_verified: !emailCodeOnlyV342,
    email_code_100_verified: emailCodeOnlyV342,
    time: now
  };
...
async function customerSecurityFinalizeRecoveryLocalWorkerV162(req, res, action, override = {}) {
  ...
  const select = 'id,request_id,customer_id,auth_user_id,email_hash,customer_binding_hash,auth_user_binding_hash,device_binding_hash,ip_hash,user_agent_hash,status,attempt_count,expires_at,used_at,revoked_at,locked_at,old_passkey_ids,metadata';
  const result = await supabaseFetch('/rest/v1/' + LOST_PASSKEY_RECOVERY_REQUEST_TABLE + '?select=' + encodeURIComponent(select) + '&request_id=eq.' + encodeURIComponent(requestId) + '&limit=2', { method: 'GET', auth: 'service' });
  ...
  if (row.used_at || row.revoked_at || row.locked_at || !Number.isFinite(expiresMs) || expiresMs <= Date.now() || String(row.status || '') !== 'verified') {
  ...
  if (finalizeVerifierModeV347) {
    if (String(metadata.binding_verifier || '') !== 'hmac_sha512_root_pepper_v347') {
      return customerSecurityLostPasskeyGenericWorkerErrorV157(res, 403, 'recovery_finalize_binding_verifier_invalid', { request_id: requestId, customer_id: owner.customerId, auth_user_id: owner.authUserId, email: owner.email });
    }
    const verifierKeyV347 = crypto.createHmac('sha512', Buffer.from(vaultSecrets.rootSecret, 'utf8'))
      .update('dirac-lost-passkey-email100-verifier-key-v347\n', 'utf8')
      .update(String(vaultSecrets.pepper || ''), 'utf8')
    ...
    let finalizeQueueTicketV350 = override && override.argonQueueTicket || null;
    let ownsFinalizeQueueTicketV350 = false;
    if (!finalizeQueueTicketV350) {
      finalizeQueueTicketV350 = await customerSecurityLostPasskeyQueueAcquireV164(req, {
        nonce: requestId,
        caller_id: 'server2_recovery_finalize_v350',
        queue_task: DIRAC_RECOVERY_WORKER_TASK_FINALIZE
      });
      ownsFinalizeQueueTicketV350 = Boolean(finalizeQueueTicketV350 && finalizeQueueTicketV350.ok);
    }
    if (!finalizeQueueTicketV350 || !finalizeQueueTicketV350.ok) {
      return res.status(finalizeQueueTicketV350 && finalizeQueueTicketV350.status || 503).json({
        ok: false,
        code: finalizeQueueTicketV350 && finalizeQueueTicketV350.code || 'RECOVERY_ARGON2_BUSY',
        message: 'Finalisasi recovery sedang diproses. Silakan coba kembali.'
      });
    }
    try {
      if (!customerSecurityLostPasskeyQueueLeaseHealthyV188(finalizeQueueTicketV350)) {
      ...
      bindingOk = await customerSecurityLostPasskeyArgon2VerifyHashV157(
        'binding', customerSecurityLostPasskeyCanonical(authoritativeBindings), metadata.binding_hash_commitment, vaultSecrets.pepper, vaultSecrets.rootSecret
      ).catch(() => false);
    } finally {
      if (ownsFinalizeQueueTicketV350) {
        try { await finalizeQueueTicketV350.release(); } catch (_) {}
      }
    }
  }
  ...
  const patched = await diracRecoveryCompareAndSetRequestV356(row, owner, { status: 'used', used_at: now, revoked_at: now, metadata: finalMetadata });
...
async function customerSecurityHandleRecoveryWorkerGenerate(req, res, action) {
  ...
  if (workerTask === DIRAC_RECOVERY_WORKER_TASK_VERIFY) {
    const verifyPasswordMaterialV342 = customerSecurityExtractPasswordMaterialV157({
      password_latest_material: body.password_latest_material || body.password_latest_proof || body.account_password || ''
    });
    if (!verifyPasswordMaterialV342) {
      await customerSecurityRegisterFailedVerification(req, 'customer_security_recovery_code_verify', 'recovery_password_missing_on_verify', owner.customerId).catch(() => null);
      return customerSecurityLostPasskeyGenericWorkerErrorV157(res, 400, 'recovery_password_missing_on_verify', body, {
        owner, bindings, requestId: String(body.request_id || ''), workerAction: DIRAC_RECOVERY_WORKER_TASK_VERIFY
      });
    }
    const verifyPasswordV342 = await customerSecurityVerifyAccountPasswordForPdfV156(owner.email, verifyPasswordMaterialV342, owner.authUserId);
    if (!verifyPasswordV342.ok) {
      await customerSecurityRegisterFailedVerification(req, 'customer_security_recovery_code_verify', 'recovery_account_password_invalid_on_verify', owner.customerId).catch(() => null);
      return customerSecurityLostPasskeyGenericWorkerErrorV157(res, 403, 'recovery_account_password_invalid_on_verify', body, {
        owner, bindings, requestId: String(body.request_id || ''), workerAction: DIRAC_RECOVERY_WORKER_TASK_VERIFY
      });
    }
...
function diracCentralTrustedClientIpV183(req) {
  return diracTrustedClientIpV356(req, process.env);
}
...
async function diracCentralInspectServiceRoleAccessV146(path, options = {}) {
  ...
  // Recovery service is not an authorized Midtrans processing surface.
  // Never inherit the payment webhook service-role exception from the primary API.
  if (ctx.action === 'midtrans_webhook') {
    return { block: true, reason: 'midtrans_webhook_forbidden_on_recovery_service_v350', status: 403 };
  }
...
function diracCentralIsUnsafeHostV146(host) {
  ...
  return !clean || require('node:net').isIP(unbracketed) !== 0
    || !/^(?:[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?\.)+[a-z]{2,63}$/.test(clean)
    || /(?:^|\.)(?:localhost|local|internal|home|test|invalid)$/.test(clean)
...
function diracRecoveryHpkeEnvGuardV159() {
  ...
  const configuredMinimumMemory = diracRecoveryHpkeEnvIntegerV159('DIRAC_RECOVERY_HPKE_ARGON2_MEMORY_KIB', DIRAC_RECOVERY_ARGON2_MEMORY_KIB_V266, 5242880);
  const minimumMemory = configuredMinimumMemory ? Math.min(configuredMinimumMemory, DIRAC_RECOVERY_ARGON2_MEMORY_KIB_V266) : 0;
...
function diracRecoveryHpkeArgon2PolicyV159(encodedHash, minimumMemory, minimumTime) {
  const parsed = customerSecurityLostPasskeyArgon2EncodedParamsV171(encodedHash);
  if (!parsed) return { ok: false };
  const memory = Number(parsed.memoryCost);
  const time = Number(parsed.timeCost);
  ...
  return {
    ok: Number.isSafeInteger(memory) && Number.isSafeInteger(time) && Number.isSafeInteger(parallelism)
      && memory >= DIRAC_RECOVERY_ARGON2_MEMORY_KIB_V266 && memory >= minimumMemory && memory <= 5242880
...
async function diracRecoveryHpkeRegisterCodeFailureV159(req, row, requestId) {
  const failure = await diracRecoveryRegisterFailureV282(
    requestId,
    'recovery_code',
    DIRAC_RECOVERY_HPKE_VERIFY_ACTION_V159
  ).catch(() => null);
  if (!failure || !['recorded', 'locked'].includes(failure.outcome)) {
    return { ok: false, locked: false };
  }
  await customerSecurityRegisterFailedVerification(
    req,
    DIRAC_RECOVERY_HPKE_VERIFY_ACTION_V159,
    failure.locked ? 'recovery_code_locked' : 'recovery_code_not_matched',
    row && row.customer_id
  ).catch(() => null);
  return { ok: true, locked: failure.locked };
}

async function diracRecoveryLinkRegisterTokenFailureV281(req, row, requestId) {
  const failure = await diracRecoveryRegisterFailureV282(
    requestId,
    'link_token',
    DIRAC_LOST_PASSKEY_RECOVERY_LINK_ACTION_V165
  ).catch(() => null);
  if (!failure || !['recorded', 'locked'].includes(failure.outcome)) return { ok: false, locked: false };
  await customerSecurityRegisterFailedVerification(
    req,
    DIRAC_LOST_PASSKEY_RECOVERY_LINK_ACTION_V165,
    failure.locked ? 'recovery_link_token_locked' : 'recovery_link_token_invalid',
    row && row.customer_id
  ).catch(() => null);
  return { ok: true, locked: failure.locked };
}
...
async function diracRecoveryHpkeSendProofV159(env, proofBody) {
  ...
  try {
    const response = await diracRecoveryEgressFetchV356(target.toString(), {
      method: 'POST',
      headers: {
...
async function diracRecoveryRegisterFailureV282(requestId, failureKind, failureSource) {
  const cleanRequestId = String(requestId || '');
  const kind = String(failureKind || '');
  const source = String(failureSource || '');
  const allowed = (kind === 'recovery_code'
      && [DIRAC_RECOVERY_HPKE_VERIFY_ACTION_V159, 'customer_security_recovery_code_verify'].includes(source))
    || (kind === 'email_code_100' && source === 'customer_security_recovery_code_verify')
    || (kind === 'link_token' && source === DIRAC_LOST_PASSKEY_RECOVERY_LINK_ACTION_V165);
  if (!/^[A-Za-z0-9_-]{16,120}$/.test(cleanRequestId) || !allowed) {
    throw DIRAC_RECOVERY_CRYPTO_V2.fail('RECOVERY_FAILURE_ARGUMENT_INVALID');
  }

  // Zero-schema CAS. The existing request row is the authority. We update only
  // attempt_count/status/locked_at and deliberately never replace metadata.
  for (let casAttempt = 0; casAttempt < 5; casAttempt += 1) {
    const read = await supabaseFetch('/rest/v1/' + LOST_PASSKEY_RECOVERY_REQUEST_TABLE
      + '?select=' + encodeURIComponent('request_id,status,attempt_count,expires_at,used_at,revoked_at,locked_at')
      + '&request_id=eq.' + encodeURIComponent(cleanRequestId)
      + '&limit=2', { method: 'GET', auth: 'service' });
    if (!read || read.ok !== true || !Array.isArray(read.data)) {
      throw DIRAC_RECOVERY_CRYPTO_V2.fail('RECOVERY_FAILURE_STORAGE_UNAVAILABLE');
    }
    if (read.data.length === 0) {
      return { attempt_count: null, locked: false, outcome: 'missing', request_id: cleanRequestId, status: null };
    }
    if (read.data.length !== 1) {
      throw DIRAC_RECOVERY_CRYPTO_V2.fail('RECOVERY_FAILURE_ROW_AMBIGUOUS');
    }
    const row = read.data[0] || {};
    const status = String(row.status || '');
    const rawAttempts = row.attempt_count;
    const currentAttempts = rawAttempts === null || rawAttempts === undefined
      ? 0
      : Number(rawAttempts);
    const expiresAtMs = Date.parse(String(row.expires_at || ''));
    if (!Number.isSafeInteger(currentAttempts)
        || currentAttempts < 0
        || currentAttempts > LOST_PASSKEY_RECOVERY_ATTEMPT_LIMIT
        || !['locked', 'pending', 'revoked', 'used', 'verified'].includes(status)) {
      throw DIRAC_RECOVERY_CRYPTO_V2.fail('RECOVERY_FAILURE_ROW_INVALID');
    }
    const active = ['pending', 'verified'].includes(status)
      && !row.used_at && !row.revoked_at && !row.locked_at
      && Number.isFinite(expiresAtMs) && expiresAtMs > Date.now();
    if (!active) {
      const rowLocked = status === 'locked' || Boolean(row.locked_at);
      return {
        attempt_count: currentAttempts,
        locked: rowLocked,
        outcome: rowLocked ? 'locked' : 'inactive',
        request_id: cleanRequestId,
        status
      };
    }

    const nextAttempts = Math.min(LOST_PASSKEY_RECOVERY_ATTEMPT_LIMIT, currentAttempts + 1);
    const locked = nextAttempts >= LOST_PASSKEY_RECOVERY_ATTEMPT_LIMIT;
    const attemptFilter = rawAttempts === null || rawAttempts === undefined
    ...
    if (patched.data.length !== 1) {
      throw DIRAC_RECOVERY_CRYPTO_V2.fail('RECOVERY_FAILURE_ROW_AMBIGUOUS');
    }
    return {
      attempt_count: nextAttempts,
      locked,
      outcome: locked ? 'locked' : 'recorded',
      request_id: cleanRequestId,
      status: locked ? 'locked' : status
    };
  }
  const finalRead = await supabaseFetch('/rest/v1/' + LOST_PASSKEY_RECOVERY_REQUEST_TABLE
    + '?select=' + encodeURIComponent('request_id,status,attempt_count,expires_at,used_at,revoked_at,locked_at')
    + '&request_id=eq.' + encodeURIComponent(cleanRequestId)
    + '&limit=2', { method: 'GET', auth: 'service' });
  const finalRows = finalRead && finalRead.ok === true && Array.isArray(finalRead.data) ? finalRead.data : [];
  const finalRow = finalRows.length === 1 ? finalRows[0] : null;
  const finalAttempts = Number(finalRow && finalRow.attempt_count);
  if (finalRow
      && String(finalRow.status || '') === 'locked'
      && finalRow.locked_at
      && Number.isSafeInteger(finalAttempts)
      && finalAttempts === LOST_PASSKEY_RECOVERY_ATTEMPT_LIMIT) {
    return { attempt_count: finalAttempts, locked: true, outcome: 'locked', request_id: cleanRequestId, status: 'locked' };
  }
  throw DIRAC_RECOVERY_CRYPTO_V2.fail('RECOVERY_FAILURE_CAS_CONFLICT');
}

function diracRecoveryResponseDecodeB64uV282(value, exactBytes, maximumBytes) {
  const clean = String(value || '');
  if (!clean || !/^[A-Za-z0-9_-]+$/.test(clean) || clean.length % 4 === 1) {
    throw DIRAC_RECOVERY_CRYPTO_V2.fail('RECOVERY_RESPONSE_ENCODING_INVALID_V282');
  }
  const decoded = Buffer.from(clean, 'base64url');
  if (decoded.toString('base64url') !== clean
      || (exactBytes !== null && decoded.length !== Number(exactBytes))
...
function diracRecoveryFinalResponseV282(value, requestId) {
  const response = value && typeof value === 'object' && !Array.isArray(value) ? value : null;
  const expected = [
    'active', 'central_guard', 'code', 'method', 'ok', 'recovery_session',
    'recovery_session_expires_at', 'request_id', 'sealed_recovery'
  ];
  const keys = response ? Object.keys(response).sort() : [];
  const sealed = response && response.sealed_recovery
    && typeof response.sealed_recovery === 'object'
    && !Array.isArray(response.sealed_recovery)
    ? response.sealed_recovery
    : null;
  const sealedKeys = sealed ? Object.keys(sealed).sort() : [];
  const expectedSealed = [
    'aead_nonce', 'ciphertext', 'owner_binding_sha512',
    'request_id', 'transcript_sha512', 'version'
  ];
  let sealedNonce = null;
  let sealedCiphertext = null;
  let sealedTranscript = null;
  let sealedOwnerBinding = null;
  try {
    if (sealed) {
      sealedNonce = diracRecoveryResponseDecodeB64uV282(sealed.aead_nonce, 12, 128);
      sealedCiphertext = diracRecoveryResponseDecodeB64uV282(sealed.ciphertext, null, 64 * 1024);
      sealedTranscript = diracRecoveryResponseDecodeB64uV282(sealed.transcript_sha512, 64, 128);
      sealedOwnerBinding = diracRecoveryResponseDecodeB64uV282(sealed.owner_binding_sha512, 64, 128);
    }
    if (!response
        ...
        || response.code !== 'RECOVERY_HYBRID_V2_VERIFIED'
        || response.central_guard !== DIRAC_CENTRAL_SECURITY_GUARD_V146
        || response.method !== 'x25519_mlkem1024_a256kw_dek_dual_signature'
        || response.request_id !== requestId
        || response.recovery_session !== null
        || response.recovery_session_expires_at !== null
        || !sealed
        || sealedKeys.length !== expectedSealed.length
        || sealedKeys.some((key, index) => key !== expectedSealed[index])
        || sealed.request_id !== requestId
        || sealed.version !== DIRAC_RECOVERY_CRYPTO_V2.RESPONSE_VERSION
        || !sealedNonce
        || !sealedCiphertext
        || sealedCiphertext.length < 17
        || !sealedTranscript
        || !sealedOwnerBinding) {
      throw DIRAC_RECOVERY_CRYPTO_V2.fail('RECOVERY_RESPONSE_INVALID_V282');
    }
    return response;
  } finally {
    if (sealedNonce) sealedNonce.fill(0);
    if (sealedCiphertext) sealedCiphertext.fill(0);
    if (sealedTranscript) sealedTranscript.fill(0);
    if (sealedOwnerBinding) sealedOwnerBinding.fill(0);
  }
}
/* source 34371-34371 */
...
async function diracRecoveryCryptoV2VerifyEnvelope(req, res, ctx, body) {
  ...
  try {
    ...
    const requestShapeValidV281 = Boolean(row && row.id
      && customerSecurityLooksLikeUuid(row.customer_id)
      && customerSecurityLooksLikeUuid(row.auth_user_id)
      && Array.isArray(row.old_passkey_ids)
      && row.old_passkey_ids.length > 0
      && typeof row.recovery_code_hash === 'string'
      && row.recovery_code_hash.length > 0);
    if (!requestShapeValidV281 || !diracRecoveryHpkeRequestActiveV159(row)) {
    ...
    const currentVaultSecretsV281 = customerSecurityLostPasskeyRequireVaultSecretsV157();
    ...
    if (!codeOk) {
      ...
      if (!failed.ok) {
        return res.status(503).json({
          ok: false,
          code: 'RECOVERY_FAILURE_STATE_UNAVAILABLE',
          message: 'Status verifikasi recovery belum dapat disimpan.'
        });
      }
    ...
    // Zero-schema mode: the main server commits the authenticated proof into
    // the existing recovery_sessions table. No auxiliary claim table is used.
    ...
    const sealedRecovery = DIRAC_RECOVERY_CRYPTO_V2.encryptResponse(
      ...
      hybrid.transcriptHash,
      {
        ...
        recoveryCode,
        recoverySessionToken: session,
        recoverySessionExpiresAt: sessionExpiresAt
      }
    );
    const finalResponseV281 = diracRecoveryFinalResponseV282({
      ok: true,
      active: true,
      ...
      recovery_session_expires_at: null,
    ...
    return res.status(200).json(finalResponseV281);
  } catch (error) {
    const code = String(error && error.code || 'RECOVERY_HYBRID_V2_FAILED');
    const status = code === 'RECOVERY_CLAIM_REJECTED' ? 409
      : code === 'RECOVERY_CLAIM_RESPONSE_EXPIRED' ? 410
      : code === 'RECOVERY_PROOF_RESPONSE_INVALID' ? 502
      : /STORAGE|CACHE|ENVIRONMENT|ABORT|LEASE_LOST|UNAVAILABLE/.test(code) ? 503
...
function diracRecoveryValidatePlainObjectV201(value, depth = 0, budget = { keys: 0, bytes: 0 }) {
  ...
  if (typeof value === 'number') {
    if (!Number.isFinite(value) || (Number.isInteger(value) && !Number.isSafeInteger(value))) throw new Error('RECOVERY_JSON_NUMBER_INVALID');
    return value;
  }
  if (typeof value === 'string') {
    if (!diracValidUnicodeV356(value)) throw new Error('RECOVERY_JSON_UNICODE_INVALID');
  ...
  for (const [key, item] of Object.entries(value)) {
    ...
    if (budget.keys > 512 || key.length > 128 || /^(?:__proto__|proto|prototype|constructor)$/i.test(key) || !diracValidUnicodeV356(key)) {
...
async function readLimitedJsonBody(req, limitBytes = 64 * 1024) {
  ...
  if (!Number.isSafeInteger(limit) || limit < 1 || limit > 2 * 1024 * 1024) {
    throw new Error('RECOVERY_BODY_LIMIT_INVALID');
  }
  const validate = (raw) => {
    const body = diracParseStrictJsonV356(raw, { maxBytes: limit, maxDepth: 12, maxNodes: 4096 });
    if (!body || typeof body !== 'object' || Array.isArray(body)) throw new Error('RECOVERY_BODY_OBJECT_REQUIRED');
    return diracRecoveryValidatePlainObjectV201(body);
  };
  try {
    // A private request cache is written only after the raw representation passes.
    if (DIRAC_RECOVERY_PARSED_BODY_V356.has(req)) return DIRAC_RECOVERY_PARSED_BODY_V356.get(req);
    let body;
    if (req && req.body !== undefined && req.body !== null) {
      // Parsed framework objects have already lost duplicate names. The endpoint
      // disables automatic parsing; do not silently accept an ambiguous object.
      body = validate(req.rawBody !== undefined ? req.rawBody : req.body);
    } else {
      const chunks = [];
      let total = 0;
      try {
        for await (const value of req) {
          ...
          total += chunk.length;
          if (total > limit) { chunk.fill(0); const e = new Error('RECOVERY_BODY_TOO_LARGE'); e.statusCode = 413; throw e; }
          chunks.push(chunk);
        }
        const raw = Buffer.concat(chunks);
        try { body = validate(raw); } finally { raw.fill(0); }
      } finally { for (const chunk of chunks) chunk.fill(0); }
    }
    DIRAC_RECOVERY_PARSED_BODY_V356.set(req, body);
    req.__diracCentralParsedBodyV146 = body;
    return body;
  } catch (error) {
    if (!error.statusCode) error.statusCode = error.code === 'JSON_SIZE_LIMIT' ? 413 : 400;
    throw error;
  }
}
const DIRAC_RECOVERY_PARSED_BODY_V356 = new WeakMap();
...
async function supabaseFetch(path, options = {}) {
  return diracRecoverySecurityDbProxyFetchV234(String(path || ''), options);
}
...
async function diracRecoveryPermanentBanV201(req, action, reason, identityKey) {
  const decision = diracRecoveryPersistentBanDecisionV281(req, action, reason, 403);
  if (!decision.persistent) return false;
  const now = Date.now();
  const blockedUntilMs = now + DIRAC_RECOVERY_PERMANENT_BAN_MS_V201;
  const key = identityKey || diracRecoveryIdentityV201(req, action);
  const record = {
    type: 'recovery_corroborated_attack_persistent_ban_v281',
    risk: 'critical',
...
function diracRecoveryPersistentBanDecisionV281(req, action, reason, status = 403) {
  const cleanReason = String(reason || 'central_guard_failed').toLowerCase().replace(/[^a-z0-9_.:-]/g, '_').slice(0, 120);
  const evidence = req && req.__diracRecoveryCorroboratedAttackV281;
  const corroboratedSignatureAttack = Boolean(
    String(action || '') === DIRAC_RECOVERY_WORKER_ACTION
    && cleanReason === 's2s_seven_signature_invalid'
    && evidence
    && evidence.reportAccepted === true
    && evidence.reportQueued === true
    && diracS2SIsCryptographicSignatureFailureEvidenceV229(evidence.evidence)
    && /^[a-f0-9]{128}$/.test(String(evidence.evidence && evidence.evidence.evidence_hash || ''))
  );
  const category = corroboratedSignatureAttack ? 'corroborated_attack'
    : Number(status) === 429 || /(?:busy|rate|queue|replay)/.test(cleanReason) ? 'bounded_rate_or_replay'
      : /(?:token_invalid|code_invalid|not_matched|credential)/.test(cleanReason) ? 'bounded_credential_failure'
        : Number(status) >= 500 || /(?:unavailable|timeout|environment|configuration|storage|runtime|lease)/.test(cleanReason) ? 'dependency_failure'
          : /(?:expired|method|media_type|content_type|origin|host|protocol|fetch|body|fields|format|binding|nonce|csrf|action)/.test(cleanReason) ? 'contract_or_browser_compatibility'
            : 'non_corroborated_policy_failure';
  return Object.freeze({
    persistent: corroboratedSignatureAttack,
    category,
    evidenceHash: corroboratedSignatureAttack ? String(evidence.evidence.evidence_hash) : ''
  });
}

async function diracRecoveryGuardRejectV201(req, res, action, reason, status = 403, identityKey = '') {
  const decision = diracRecoveryPersistentBanDecisionV281(req, action, reason, status);
  if (!decision.persistent) {
    const responseCode = Number(status) === 429 ? 'RECOVERY_RATE_LIMITED'
      : Number(status) >= 500 ? 'RECOVERY_GUARD_DEPENDENCY_UNAVAILABLE'
        : 'RECOVERY_CENTRAL_GUARD_REJECTED';
...
function diracRecoveryAssertServer2EnvironmentV201() {
  ...
  if (process.env.NODE_ENV === 'production') {
    ...
    const previousRootSecretPresentV281 = String(process.env.DIRAC_SECURITY_ROOT_SECRET_PREVIOUS || '').length > 0;
    const previousRootVersionV281 = String(process.env.DIRAC_SECURITY_ROOT_SECRET_PREVIOUS_VERSION || '').trim();
    if (previousRootSecretPresentV281 || previousRootVersionV281) {
      const previousSecretsV281 = customerSecurityLostPasskeySecretsForMetadataV281(
        { root_secret_version: previousRootVersionV281 },
        vaultSecrets
      );
      if (!previousSecretsV281.ok || previousSecretsV281.rootSecretVersion !== previousRootVersionV281) {
        throw new Error('DIRAC_SERVER2_PREVIOUS_ROOT_SECRET_KEYRING_INVALID');
      }
    }
  }
  return true;
...
function diracRecoveryLinkOpenArgonPolicyV202(metadata) {
  ...
  const linkDeclaredCandidate = source.link_token_argon2id_params && typeof source.link_token_argon2id_params === 'object' && !Array.isArray(source.link_token_argon2id_params)
    ...
    : null;
  const linkDeclaredKeys = linkDeclaredCandidate ? Object.keys(linkDeclaredCandidate).sort() : [];
  const linkDeclaredHasHistoricalShape = linkDeclaredKeys.join(',') === 'memoryCost,parallelism,timeCost';
  const linkDeclaredHasHashLengthShape = linkDeclaredKeys.join(',') === 'hashLength,memoryCost,parallelism,timeCost';
  const linkDeclared = linkDeclaredCandidate
    && (linkDeclaredHasHistoricalShape || linkDeclaredHasHashLengthShape)
    ...
    && (!linkDeclaredHasHashLengthShape || Number(linkDeclaredCandidate.hashLength) === 64)
    ? {
        memoryCost: Number(linkDeclaredCandidate.memoryCost),
        timeCost: Number(linkDeclaredCandidate.timeCost),
        parallelism: Number(linkDeclaredCandidate.parallelism),
        ...(linkDeclaredHasHashLengthShape ? { hashLength: 64 } : {})
      }
    : null;
  ...
  if (!linkDeclared && metadataVaultProfile && bundleVaultProfile && signedVaultProfile
    ...
    const currentVaultSecretsV281 = customerSecurityLostPasskeyRequireVaultSecretsV157();
    const vaultSecrets = customerSecurityLostPasskeySecretsForMetadataV281(source, currentVaultSecretsV281);
...
async function diracRecoveryLinkOpenGuardV202(req, res, ctx, body, identityKey) {
  ...
  if (host !== new URL(diracRoleOriginV250('security')).hostname) {
...
async function diracRecoveryLinkOpenV202(req, res, ctx, body) {
  ...
  try {
    ...
    if (!tokenOk) {
      ...
      if (!failure.ok) {
        return diracRecoveryLinkOpenJsonV202(res, 503, 'RECOVERY_LINK_FAILURE_STATE_UNAVAILABLE', 'Layanan recovery belum siap.');
      }
      try { res.setHeader('Retry-After', '300'); } catch (_) {}
      return diracRecoveryLinkOpenGenericInvalidV202(res);
    }
...
function diracS2SServer1TargetV206() {
  const raw = diracRecoveryHpkeServer1UrlV159();
  if (!raw) return null;
...
async function diracS2SSendSecurityReportV206(payload) {
  ...
  try {
    ...
    let data = {};
    try { data = text ? diracParseStrictJsonV356(text, { maxBytes: 32 * 1024 }) : {}; } catch (_) { data = {}; }
...
async function diracS2SCheckCentralRevocationV206(serverId, keyVersion) {
  ...
  if (!result.ok || !result.data || result.data.event !== 'revocation_check' || typeof result.data.revoked !== 'boolean') return { ok: false, unavailable: true };
...
async function diracRecoveryWorkerGuardV201(req, res, ctx, body, identityKey) {
  ...
  if (!sevenSignatureVerification.ok) {
    if (sevenSignatureVerification.reason === 's2s_seven_signature_invalid') {
      const report = await diracS2SQueueAndReportFailureV206(
        ctx,
        sevenSignatureVerification,
        sevenSignatureVerification.reason
      ).catch(() => null);
      if (report
          && report.ok === true
          && report.queued === true
          && report.attributed === true
          && diracS2SIsCryptographicSignatureFailureEvidenceV229(report.evidence)) {
        req.__diracRecoveryCorroboratedAttackV281 = {
          reportAccepted: true,
          reportQueued: true,
  ...
  try {
    ...
    ctx.body = diracRecoveryValidatePlainObjectV201(transportContext.body);
    DIRAC_RECOVERY_PARSED_BODY_V356.set(req, ctx.body);
...
async function diracRecoveryPreflightGuardV204(req, res, ctx, identityKey) {
  ...
  const secFetchSite = diracRecoveryHeaderV201(req, 'sec-fetch-site').toLowerCase();
  if (origin !== DIRAC_RECOVERY_BROWSER_ORIGIN_V201
      || host !== new URL(diracRoleOriginV250('security')).hostname
      || (process.env.NODE_ENV === 'production' ? forwardedProto !== 'https' : (forwardedProto && forwardedProto !== 'https'))
...
module.exports = async function diracServer2StrictRecoveryBoundaryV221(req, res) {
  const requestBoundary = diracRecoveryRequestBoundaryV356(req);
  if (!requestBoundary.ok) return res.status(requestBoundary.status).json({ ok: false, code: requestBoundary.code, message: 'Permintaan recovery tidak valid.' });
  try {
    diracServer2StrictAssertV221();
  } catch (_) {
...
supabaseFetch = async function supabaseFetchViaServer1SecurityProxyV234(path, options = {}) {
  ...
  return diracRecoverySecurityDbProxyFetchV234(cleanPath, options);
};
...
diracV107DirectFetch = async function diracV107DirectFetchViaServer1SecurityProxyV234(method, suffix, body) {
  const table = diracV107Table();
  if (!table) return diracRecoverySecurityDbProxyFailureV234('RECOVERY_SECURITY_DB_PROXY_TABLE_INVALID');
...
diracS2SSendSecurityReportV206 = async function diracS2SSendSecurityReportDiagnosticV236(payload) {
  ...
  try {
    ...
    try { data = text ? diracParseStrictJsonV356(text, { maxBytes: 32 * 1024 }) : {}; }
...
async function diracRecoveryEgressFetchV356(input, options = {}) {
  const raw = String(input || '');
  if (/[\u0000-\u0020\u007f\\]/.test(raw)) throw new Error('RECOVERY_EGRESS_URL_INVALID');
  const target = new URL(raw);
  if (target.protocol !== 'https:' || target.username || target.password || target.hash
      || target.port || diracCentralIsUnsafeHostV146(target.hostname)) throw new Error('RECOVERY_EGRESS_DESTINATION_DENIED');
  const method = String(options.method || 'GET').toUpperCase();
  const server1 = diracRecoveryHpkeServer1UrlV159();
  const upstream = server1 ? new URL(server1) : null;
  const parameters = Array.from(target.searchParams.keys());
  const allowedServer = upstream && target.origin === upstream.origin && target.pathname === '/api/health'
    && parameters.length === 1 && parameters[0] === 'action'
    && ['security_report', DIRAC_RECOVERY_HPKE_PROOF_ACTION_V159].includes(target.searchParams.get('action'));
  const allowedEmail = target.href === 'https://api.resend.com/emails' || target.href === 'https://api.brevo.com/v3/smtp/email';
  if (method !== 'POST' || (!allowedServer && !allowedEmail)) throw new Error('RECOVERY_EGRESS_OPERATION_DENIED');
  if (typeof globalThis.fetch !== 'function' || typeof AbortSignal.timeout !== 'function') throw new Error('RECOVERY_EGRESS_RUNTIME_UNAVAILABLE');
  const timeout = AbortSignal.timeout(15_000);
  const signal = options.signal ? AbortSignal.any([options.signal, timeout]) : timeout;
  return globalThis.fetch(target.href, { ...options, method, signal, redirect: 'error' });
}

// Frameworks supporting API route configuration must preserve the raw JSON bytes.
module.exports.config = { api: { bodyParser: false } };

function diracRecoveryRequestBoundaryV356(req) {
  const deny = (code, status = 400) => ({ ok: false, status, code });
  if (!req || !['POST', 'HEAD', 'OPTIONS'].includes(req.method)) return deny('RECOVERY_METHOD_NOT_ALLOWED', 405);
  ...
  if (!Array.isArray(rawHeaders) || rawHeaders.length % 2) return deny('RECOVERY_HEADER_AMBIGUOUS');
  for (let index = 0; index < rawHeaders.length; index += 2) {
    const name = String(rawHeaders[index] || '').toLowerCase();
    if (!name || seen.has(name) || /[\r\n\0]/.test(String(rawHeaders[index + 1] || ''))) return deny('RECOVERY_HEADER_AMBIGUOUS');
    seen.add(name);
  }
  if (headers['content-length'] !== undefined && !/^(?:0|[1-9][0-9]{0,6})$/.test(String(headers['content-length']))) return deny('RECOVERY_LENGTH_INVALID');
  if (headers['content-length'] !== undefined && headers['transfer-encoding'] !== undefined) return deny('RECOVERY_LENGTH_AMBIGUOUS');
  return { ok: true };
}

function diracRecoveryRequestTransportV356(req, env = process.env) {
  if (req && req.socket && req.socket.encrypted === true) return true;
  if (diracRecoveryHeaderV201(req, 'x-forwarded-proto') !== 'https') return false;
  const mode = String(env.DIRAC_TRUSTED_PROXY_MODE || (env.VERCEL === '1' ? 'vercel' : 'none')).trim().toLowerCase();
  if (mode === 'vercel') return env.VERCEL === '1';
  if (mode !== 'standard') return false;
  const raw = String(env.DIRAC_TRUSTED_PROXY_IPS || '');
  if (!raw || raw.length > 4096) return false;
  const configured = raw.split(',').map((item) => item.trim()).filter(Boolean).map(diracNormalizeIpV356);
  const peer = diracNormalizeIpV356(req && req.socket && req.socket.remoteAddress || '');
  return Boolean(peer && configured.length && configured.every(Boolean) && configured.includes(peer));
}
